...
| 层 | 功能 | 核心组件 |
|----|------|---------|
| **界面层** | 三栏 UI + 多渠道接入 | SwarmWS 资源管理器、Chat Center（1-4 标签页）、Swarm Radar、渠道网关（Slack/飞书） |
| **智能层** | 主动感知 + 自主执行 | 主动智能（L0-L4）、信号管道、自主管道（8 阶段）、任务系统 |
| **Harness 层** | 核心创新——将原始 Claude 变成 Agentic OS | 上下文工程（11 文件）、记忆管道（三层）、自我进化（55+ 技能）、安全 + 自检 |
...
- **55+ 内置技能** —— 浏览器自动化、PDF 处理、电子表格、Slack、Outlook、Apple 提醒、网页研究、代码审查、自主管道等
...
| 飞轮 | 功能 | 核心组件 |
|------|------|---------|
| **自我进化** | 构建新技能、捕获纠正、永不重犯 | EVOLUTION.md、55+ 技能、缺口检测、纠正注册表 |
| **自我记忆** | 三层蒸馏管道，git 验证，LLM 驱动的周维护 | DailyActivity、蒸馏钩子、MEMORY.md、主动简报 |
...
| **L2** | **自我改进**（当前） | LLM 周维护、统一任务系统、反馈闭环完成 |
| L3 | 自我治理（进行中） | 上下文按会话类型适配、主动缺口检测、DDD 自动同步 |
| L4 | 自主运行 | 完整 AIDLC 管道、自主学习、判断力框架 |

这不是功能清单——是成长架构。每次会话让下一次更好。每次纠正防止一类未来错误。系统不只是运行；它在**复利**。

### 5. Swarm Brain —— 一个 AI，所有渠道，共享记忆

Swarm 是个人助手。它只有**一颗大脑**。无论你通过聊天标签页、Slack、飞书还是任何未来的渠道跟它对话——都是同一个 Swarm，同一份记忆，同一个上下文。
...
| **技能** | 55+ 内置（浏览器、PDF、Slack、Outlook、研究...） | 仅工具调用 |
| **自主管道** | 8 阶段生命周期 + ROI 门控 + 升级协议 + 制品链 | 手动工作流 |
| **多渠道** | 桌面 + Slack + 飞书（统一大脑） | 仅终端 |
...
| **渠道** | 桌面 + Slack + 飞书（统一大脑——所有渠道共享一个会话） | 21+ 即时通讯平台（渠道间隔离） |
| **技能** | 55+ 精选 + 自建 | 5,400+ 市场 |
