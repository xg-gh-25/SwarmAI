...
我们终于有了聪明到能推理、写代码、做判断的软件——可它**每天早上醒来都失忆。** 每个会话从零开始：你给过的上下文、它昨天犯的错、你纠正过的判断——全部归零。大多数"AI 工具"就是一个聪明的模型，困在土拨鼠之日里。

SwarmAI 押的是相反的赌注：价值应该**复利**。每一次交互都让系统比之前更敏锐一点，且是永久的。

模型只是**回答**；**心智会持续**。它跨时间连续、自我纠错、遗忘掉不再重要的东西、判断力随使用复利。普通软件这四样一样都没有——程序不会在两次运行之间变聪明，也不会改写自己的规则。SwarmAI 就是那个缺失的层：不是更大的模型，而是**它周围的操作系统。**
...
大多数 agent memory 项目都在堆条目。SwarmAI 把**认知**（OS）和**知识**（硬盘）分开：改 `SOUL.md` 一行，比加一千条 memory 更能改变判断——而且每次改动都是一个 `git diff`。一个复发的错误，不会变成又一条 lesson，而是变成一道**门控**——一条让错误动作无法发生的路径。不是空话：`security_hooks.py` 里有 20+ 道实时 guard（commit 门、pytest 守卫、危险命令门）。点进去读——错误动作是代码里挡的，不是写在 guideline 里。衡量进步的不是纠正数量的增长，而是某一类错误**停止复发**。
