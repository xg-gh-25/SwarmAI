...
这就重新框定了在造的东西。模型只是**回答**；**心智会持续**。四件事把两者区分开：它跨时间**连续**、它**自我纠错**、它**遗忘**掉不再重要的东西、它的**判断力随使用复利**。传统软件对这四样没有任何对应物——程序不会在两次运行之间变聪明，更不会改写自己的规则。SwarmAI 就是那个缺失的层：不是更大的模型，而是**它周围的认知操作系统。**

---

## 三个核心理念

SwarmAI 里的一切，都服务于其中之一：

### 🧬 自进化 —— 它升级自己的判断力

大多数 agent memory 项目都在堆条目。SwarmAI 把**认知**（OS）和**知识**（硬盘）分开：改 `SOUL.md` 一行，比加一千条 memory 更能改变判断——而且每次改动都是一个 `git diff`。纠正不是记成更多 lesson；一个复发的错误类别会变成一道**结构性门控**，一条让错误动作物理上无法发生的路径。人类靠 carefulness，agent 该靠 structure。衡量进步的不是纠正数量的增长——而是某一类错误**停止复发**。

### 🧠 Brain-first —— 每个项目都是一个领域脑

项目不是一堆文件——它是一个**脑**，有一套通用的六段结构（身份 · 知识 · 门控 · 能力 · 交付 · 刷新），**对所有用户、所有领域都一样**。唯一变化的是它管理什么：`0..N` 个开放类别的资产（一个代码库、一个数据源、一个文档语料、一个流程——或者什么都不管）。一个代码库、一个研究课题、一个顾问的客户，甚至"我的婚礼"，拿到的都是**同一个脑**。知识随你工作自动沉淀进去，不再重要的知识会**衰减、死亡**——只积累不淘汰，是所有 memory 系统腐烂的根因。

### ⚙️ Agent OS —— 认知活在会话之间，不在会话之内

会话是离散的，智能不该是。Hooks 在会话*之间*触发，让下一个会话热启动。会话崩了系统自愈，知识按节奏自动培育与衰减，每个会话的系统提示都从受治理的上下文文件里重新组装。
...
| **Eval（本体感觉）** | 解耦、系统级：Golden Set + git 绑定回归门控 | [docs](./docs/OS-Eval-Function-Design.md) |
...
如果你是在这个 repo 里工作的 AI 编码助手，从 **[`AGENTS.md`](./AGENTS.md)** 开始 —— 一份结构化的架构指南：数据流、进程拓扑、约定与不变量。它是面向 agent 的入口（这份 README 是面向人的入口）。
