...
> **⚠️ Post-Refactor Update**: MCP binding is now direct — the agent's `mcp_ids` are looked up and used directly, with no workspace intersection model. `WorkspaceConfigResolver` was removed entirely.
```
...
> **⚠️ Post-Refactor Update**: The unified-swarm-workspace-cwd refactor replaced per-agent workspace rebuilds with `AgentSandboxManager.setup_workspace_skills()`, which handles skill symlinks at app init and on skill CRUD events, shared across all agents in the unified SwarmWorkspace.
...
> - **`WorkspaceConfigResolver` removed**: MCP configuration uses agent's `mcp_ids` directly with no workspace intersection model.
