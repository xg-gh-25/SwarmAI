# SwarmAI Desktop Build Guide

This guide details how to build the SwarmAI Mac/Linux desktop application.
...
├── dmg/
│   └── SwarmAI_1.0.0_aarch64.dmg  # DMG installer
└── macos/
    └── SwarmAI.app/               # Application bundle
```
...
desktop/src-tauri/target/release/bundle/
├── deb/
│   └── swarmai_1.0.0_amd64.deb   # Debian package
└── appimage/
    └── swarmai_1.0.0_amd64.AppImage  # AppImage
```
...
1. **Option 1: Using DMG installer (Recommended)**
   ```bash
   # Open DMG file
   open desktop/src-tauri/target/release/bundle/dmg/SwarmAI_1.0.0_aarch64.dmg

   # In the opened window, drag SwarmAI.app to the Applications folder
   # Then launch from Launchpad or Applications folder
   ```

2. **Option 2: Run .app directly**
   ```bash
   # Open application bundle directly
   open desktop/src-tauri/target/release/bundle/macos/SwarmAI.app
   ```

3. **First run notes**

   Since the app is not signed by Apple, you may see "Cannot open because the developer cannot be verified". Solution:

   - Right-click `SwarmAI.app` → Select "Open" → Click "Open" in the dialog

   Or run in terminal:
   ```bash
   xattr -cr /Applications/SwarmAI.app
   open /Applications/SwarmAI.app
   ```
...
1. **Debian/Ubuntu (.deb)**
   ```bash
   # Install
   sudo dpkg -i desktop/src-tauri/target/release/bundle/deb/swarmai_1.0.0_amd64.deb
...
2. **AppImage (universal)**
   ...
   chmod +x desktop/src-tauri/target/release/bundle/appimage/swarmai_1.0.0_amd64.AppImage

   # Run directly
   ./desktop/src-tauri/target/release/bundle/appimage/swarmai_1.0.0_amd64.AppImage
   ```
...
1. Check log files (`~/Library/Logs/SwarmAI/` or terminal output)
2. Check the [FAQ](#常见问题) section
