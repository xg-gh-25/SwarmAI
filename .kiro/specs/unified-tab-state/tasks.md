...
- [ ] 1. Implement `useUnifiedTabState` hook
  - [ ] 1.1 Create `desktop/src/hooks/useUnifiedTabState.ts` with `UnifiedTab`, `SerializableTab`, `TabStatus` types and `UseUnifiedTabStateReturn` interface
    - Define all types and interfaces from the design document
    - Export `TabStatus` type for downstream consumers
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5, 1.6_

  - [ ] 1.2 Implement Tab_Map, Render_Counter, activeTabId state, and localStorage initialization
  ...
  - [ ] 1.3 Implement `useMemo` derived views: `openTabs`, `tabStatuses`, `activeTab`
  ...
  - [ ] 1.4 Implement tab CRUD methods: `addTab`, `closeTab`, `selectTab`
  ...
  - [ ] 1.5 Implement metadata update methods: `updateTabTitle`, `updateTabSessionId`, `setTabIsNew`
    - Guard with `Map.has(tabId)` — no-op if tab doesn't exist
    - Increment Render_Counter only on actual mutation
    - _Requirements: 3.1, 3.2, 3.3, 3.4_

  - [ ] 1.6 Implement runtime state methods: `getTabState`, `updateTabState`, `updateTabStatus`
  ...
  - [ ] 1.7 Implement lifecycle methods: `saveCurrentTab`, `restoreTab`, `initTabState`, `cleanupTabState`
  ...
  - [ ] 1.8 Implement `removeInvalidTabs` and localStorage persistence effect
  ...
  - [ ] 1.9 Assemble and export the hook return object with all methods and refs
...
- [ ] 2. Checkpoint - Verify hook implementation
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Property-based and unit tests for `useUnifiedTabState`
  - [ ]* 3.1 Write property test: Tab Operation Invariants
  ...
  - [ ]* 3.2 Write property test: Per-Tab State Isolation
  ...
  - [ ]* 3.3 Write property test: addTab Produces Correct Defaults
  ...
  - [ ]* 3.4 Write property test: closeTab Removes and Reselects
  ...
  - [ ]* 3.5 Write property test: Metadata Updates Apply to Correct Tab
  ...
  - [ ]* 3.6 Write property test: localStorage Persistence Round-Trip
  ...
  - [ ]* 3.7 Write property test: removeInvalidTabs Resets Stale Tabs
...
- [ ] 4. Checkpoint - Verify all hook tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Wire `useUnifiedTabState` into ChatPage and streaming lifecycle
  - [ ] 5.1 Update `desktop/src/pages/ChatPage.tsx` to use `useUnifiedTabState` instead of `useTabState`
  ...
  - [ ] 5.2 Update `desktop/src/hooks/useChatStreamingLifecycle.ts` to consume unified hook instead of owning tab state
...
- [ ] 6. Checkpoint - Verify ChatPage integration works
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Update downstream consumers and delete legacy code
  - [ ] 7.1 Update `desktop/src/pages/chat/components/ChatHeader.tsx` to import `TabStatus` from `useUnifiedTabState`
    - Replace any `TabStatus` import from `useChatStreamingLifecycle` with import from `useUnifiedTabState`
    - _Requirements: 9.3_

  - [ ] 7.2 Update `desktop/src/pages/chat/components/SessionTabBar.tsx` to import from `useUnifiedTabState`
    - Update any tab-related type imports to reference the unified hook module
    - _Requirements: 9.3_

  - [ ] 7.3 Update `desktop/src/pages/chat/components/TabStatusIndicator.tsx` to import `TabStatus` from `useUnifiedTabState`
    - Replace any `TabStatus` import from `useChatStreamingLifecycle` with import from `useUnifiedTabState`
    - _Requirements: 9.3_

  - [ ] 7.4 Delete `desktop/src/hooks/useTabState.ts`
