...
> **Added after all 4 cadences were implemented. These notes document known gaps between this spec and the backend folder structure.**

### Gap: Semantic zones reference `Knowledge/` folder that backend does not create (🔴 High)

**This spec defines:** Two semantic zones — "Shared Knowledge" mapping to `['Knowledge']` and "Active Work" mapping to `['Projects']`. The `VirtualizedTree.tsx` `SEMANTIC_ZONES` constant was implemented exactly per this spec.

**Backend (Cadence 1) creates:** `Signals, Plan, Execute, Communicate, Reflection, Artifacts, Notebooks, Projects, Knowledge/Memory`. There is no top-level `Knowledge/` directory — only `Knowledge/Memory` (a nested path under a parent that doesn't exist as a standalone folder in `FOLDER_STRUCTURE`).

**Impact:** The "Shared Knowledge" zone is always empty in the explorer. The Operating Loop folders (Signals, Plan, etc.) and Artifacts/Notebooks fall into the "remaining top-level directories" bucket in the flattening algorithm, appearing below the semantic zones without grouping.

**Resolution:** A folder structure migration spec is needed to align the backend `FOLDER_STRUCTURE` with this spec's semantic zones. See `swarmws-foundation` reconciliation notes for details.
