...
- [x] 1. Add context assembly schemas and chat thread schema updates
  - [x] 1.1 Create `backend/schemas/context.py` with Pydantic models
  ...
  - [x] 1.2 Update `backend/schemas/chat_thread.py` to add `project_id` and `context_version` fields
...
- [x] 2. Update database layer for chat thread project association and version tracking
  - [x] 2.1 Update `backend/database/sqlite.py` to add `project_id` and `context_version` columns to `chat_threads` table
  ...
  - [x] 2.2 Add `list_by_project()`, `list_global()`, `bind_thread()`, and `increment_context_version()` methods to `SQLiteChatThreadsTable`
  ...
  - [x] 2.3 Write unit tests for chat thread project_id schema, queries, and binding in `backend/tests/test_chat_thread_project.py`
...
- [x] 3. Implement the 8-layer context assembly engine
  - [x] 3.1 Create `backend/core/context_assembler.py` with `ContextAssembler`, `ContextLayer`, `AssembledContext`, and `TruncationInfo`
    - Include module-level docstring per project code documentation standards
  ...
  - [x] 3.2 Implement L0 tag-based filtering methods in `ContextAssembler`
  ...
  - [x] 3.3 Implement layer loading methods in `ContextAssembler`
  ...
  - [x] 3.4 Implement 3-stage progressive truncation and assembly orchestration
  ...
  - [x] 3.5 Write property test: Context assembly priority ordering (`backend/tests/test_context_assembler.py`)
  ...
  - [x] 3.6 Write property test: Tag-based L0 fast-filter gating (`backend/tests/test_context_assembler.py`)
  ...
  - [x] 3.7 Write property test: Token budget invariant (`backend/tests/test_context_assembler.py`)
  ...
  - [x] 3.8 Write property test: Progressive truncation respects priority with summary (`backend/tests/test_context_assembler.py`)
  ...
  - [x] 3.9 Write property test: Layer 2 bounding invariant (`backend/tests/test_context_assembler.py`)
  ...
  - [x] 3.10 Write property test: Deterministic assembly (`backend/tests/test_context_assembler.py`)
  ...
  - [x] 3.11 Write property test: Stable project pathing (`backend/tests/test_context_assembler.py`)
...
- [x] 4. Implement context snapshot cache
  - [x] 4.1 Create `backend/core/context_snapshot_cache.py` with `ContextSnapshotCache`, `VersionCounters`, `CacheEntry`
    - Include module-level docstring per project code documentation standards
  ...
  - [x] 4.2 Add version counter increment hooks to existing managers
  ...
  - [x] 4.3 Write property test: Cache correctness (`backend/tests/test_context_snapshot_cache.py`)
  ...
  - [x] 4.4 Write unit tests for cache in `backend/tests/test_context_snapshot_cache.py`
...
- [x] 6. Implement context preview API endpoint with ETag caching
  - [x] 6.1 Create `backend/routers/context.py` with `GET /api/projects/{project_id}/context` endpoint
    - Include module-level docstring per project code documentation standards
  ...
  - [x] 6.2 Register the context router in `backend/main.py`
    - Import and include the new context router
    - _Requirements: 33.1_

  - [x] 6.3 Write unit tests for context preview API in `backend/tests/test_context_preview_api.py`
    - Include module-level docstring describing what is tested
  ...
  - [x] 6.5 Write property test: Token count consistency (`backend/tests/test_context_preview_api.py`)
...
- [x] 7. Implement chat thread project association and mid-session binding
  - [x] 7.1 Update `backend/core/chat_thread_manager.py` to support `project_id` and binding
  ...
  - [x] 7.2 Update `backend/routers/chat.py` to accept `project_id` in thread creation and add binding/project-filtered endpoints
  ...
  - [x] 7.3 Write property test: Chat thread project_id semantics (`backend/tests/test_chat_thread_project.py`)
  ...
  - [x] 7.4 Write property test: Thread binding increments version (`backend/tests/test_chat_thread_project.py`)
...
- [x] 9. Add frontend types and context service
  - [x] 9.1 Add TypeScript interfaces to `desktop/src/types/index.ts`
  ...
  - [x] 9.2 Create `desktop/src/services/context.ts` with `getContextPreview()`, `bindThread()`, and `toCamelCase` conversion
  ...
  - [x] 9.3 Write unit tests for context service in `desktop/src/services/__tests__/context.test.ts`
...
- [x] 10. Implement context preview panel frontend component
  - [x] 10.1 Create `desktop/src/components/workspace/ContextPreviewPanel.tsx`
    - Include file-level JSDoc comment per project code documentation standards
  ...
  - [x] 10.2 Write unit tests for `ContextPreviewPanel` in `desktop/src/components/__tests__/ContextPreviewPanel.test.tsx`
...
- [x] 11. Integrate context assembler into AgentManager
  - [x] 11.1 Update `backend/core/agent_manager.py` `_build_system_prompt()` to use `ContextAssembler` via `ContextSnapshotCache`
