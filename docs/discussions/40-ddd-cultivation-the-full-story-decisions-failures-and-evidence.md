...
```
User: "Add the ClientOrg monthly report"
...
This isn't hypothetical. We have 9 ClientOrg skills that use DDD context. Before DDD was populated, each skill took 3-5 iterations to get the SQL right. After: first-attempt accuracy for domain-specific logic.
...
Example: Pipeline delivers a ClientOrg monthly report, discovers `territory_owner` column doesn't exist (TECH.md pitfall). Next week, Pollinate generates GTM content for the same BU — it won't reference territory_owner data because TECH.md now says it doesn't exist.
