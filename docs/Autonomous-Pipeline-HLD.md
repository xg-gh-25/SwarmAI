...
`cross_boundary`; REVIEW reads it too). Run state lives in `run.json`; the durable brain
...
| Adversarial sub-agent spawn is rejected by the harness | Retry exactly once | Still rejected → checkpoint `gate_spawn_blocked`; resume re-enters on a fresh subprocess (never fall back to self-review, that is the bypass the gate exists to prevent) |
...
corrections; the projection layer ships the skill; the Job System runs it in the
...
the narrative-writing skill; the pipeline routinely runs on its own code). No feature flag,
...
|---|----------|--------------------|
| Q1 | The diagrams (`diagrams-pipeline/*.svg`, Figure 1 v4) predate the RP-range and TEST-4-layer calibration, do any hard-code a now-stale count or a 3-layer TEST? | Audit each SVG against the calibrated text; re-render only those that pin a drifted number. Track as a follow-up, not a blocker (the prose is the SSOT; the figures illustrate). |
