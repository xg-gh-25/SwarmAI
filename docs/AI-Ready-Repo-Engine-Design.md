...
| **Behavior coverage** (blind-spot: every behavior of a span is documented, §11.2) | ⚠️ report shipped, no gate | ⚠️ report shipped, no gate | ❌ report degrades — keys off `risk_areas`/`hot_zones` the AST layer can't extract on non-parseable legacy |
...
This closes the reverse direction: §11.1 asks "is every anchor in the spec?"; §11.2 asks "is every *behavior* of each anchored span in the spec?" Coverage is only real when both hold. **Status (2026-07-19, `35e1b1c5`): the REPORT-ONLY reverse scan is shipped and wired** — `blind_spot_scan` (`ai_ready_helpers.py:948`) keys off deterministic `risk_areas`/`hot_zones` (real fan-in/risk facts, NOT an LLM "X does not exist" assertion) and asks, for each risky span, "does any `steps[].file_path` OR `business_rules[].anchor` document the file that owns it?" — emitting a per-package `BLIND-SPOTS.md` (`render_blind_spots_md`, wired via INSTRUCTIONS.md §4.9). The **fail-closed `behavior_coverage` gate remains deliberately deferred** (C042: never BLOCK a run on an LLM negative assertion over sparse step data) — the shipped piece is an honest, non-silent *report*, not a gate.
...
| ④ | **Adversarial spec validation** (test the SPEC, not the code) | over-promises, contradictions, specs too vague to catch errors | ⚠️ partial — restraint skeleton ✅ shipped (§11.4a); **Blind-Spot reverse-coverage ✅ shipped as a report** (§11.4c, `35e1b1c5`); Contradiction vacuous-deferred (b), False-Promise/Weak-Spec runtime-deferred (d). Our Gate-2 adversarial sub-agent remains single-direction; the full bidirectional 4-detector framework (§11.4) is the tracked upgrade. |
...
**Adoption status (evidence-based, run_d9bd27da / Run D — a Gate-0 skeptic tested every detector against the REAL `code-intel.json` data before deciding what to build):**

- **(a) Restraint skeleton → ✅ SHIPPED** (Run 6, `s_autonomous-pipeline/stages/deliver.md:401`): the 3-question threshold, the "Do NOT report" negative list, and "ZERO findings is valid only after reading every changed file" are live in the Gate-2 adversarial prompt. This was the highest-leverage, zero-mechanism item and it is done.

- **(b) Contradiction 7-class → NOT BUILT (deliberately — would be an empty gate today).** The 7-class taxonomy is inherently *semantic* (deciding two free-text rules contradict requires understanding their prose = the LLM-judge boundary this engine refuses to cross statically, §4.5). The only *mechanical* subset — two `verified:true` rules on the **same `anchor`**, or two `step.contract`s on the **same HTTP path** with conflicting `status_codes` — was run against the flagship repo and found **0 instances**: the 6 `business_rules` point at 6 distinct files, the 4 contracts at 4 distinct paths, no collisions. Shipping a gate whose firing condition is structurally absent from the data is mechanism-for-its-own-sake — the exact anti-pattern the restraint skeleton (rule 5) warns against. **Activation trigger:** build the mechanical collision-subset only once data density supports non-vacuous firing (multiple rules per anchor, or contracts sharing a path). The semantic 7-class version is a future LLM-consumer-API, not part of the static tool.

- **(c) Blind-Spot (§11.2) → REPORT SHIPPED (`35e1b1c5`, 2026-07-19); fail-closed gate still deferred.** The *report-only* reverse scan is now built and wired: `blind_spot_scan` (`ai_ready_helpers.py:948`) keys off deterministic `risk_areas`/`hot_zones` (real fan-in/risk facts — NOT an LLM "does-not-exist" assertion) and flags any risky code span whose owning file is documented by no `steps[].file_path` and no `business_rules[].anchor`; output is a per-package `BLIND-SPOTS.md` (`render_blind_spots_md`, INSTRUCTIONS.md §4.9). This closes the "wire a dead function" gap (it had a full impl + tests but zero callers). What is **still deferred (deliberately, C042):** the *fail-closed* `behavior_coverage` gate that would BLOCK a run — gating an LLM negative assertion over sparse step data stays a report, not a gate. The deeper semantic detector (spec vs. the code's *actual runtime* behaviors) still needs runtime/LLM and is out of the static tool's scope. **Activation trigger for the deeper form:** populate `step.entry_ref` at generation + a runtime observation source.

- **(d) False-Promise (dynamic) + Weak-Spec (mutation) → DEFERRED (runtime, already codified design-only).** Both require *executing* the target repo — the Run C wall (a static analyzer never produces runtime observations; see the §7 equivalence layer note codified at `ai_ready_helpers.py` `derive_equivalence_assertions`/`score_equivalence`, which returns `unchecked` rather than fabricate). They are a **consumer-API** awaiting an external CI/test-runner that supplies real observations — not something the static tool builds.

**The honest one-line summary (updated 2026-07-19):** the restraint skeleton is shipped (a); **Blind-Spot now ships as a REPORT** (c, `35e1b1c5` — per-package `BLIND-SPOTS.md`), with only its *fail-closed gate* form still deferred (C042); Contradiction's mechanical collision-subset stays deferred as vacuous on today's data (b); False-Promise-dynamic + Weak-Spec fundamentally require runtime and are deferred as a consumer-API (d). Net: 1 shipped + 1 report-shipped/gate-deferred + 1 vacuous-deferred + 2 runtime-deferred. Building the vacuous/runtime pieces now would be C042 over-engineering; shipping the report (deterministic, non-silent) and documenting the rest with activation triggers was the correct deliverable.
...
| Behavior coverage (blind-spot) | ⚠️ **report ✅ shipped** (`blind_spot_scan` → per-package `BLIND-SPOTS.md`, `35e1b1c5`); fail-closed `behavior_coverage` gate deferred (C042) | the report surfaces risky spans no step/rule documents (deterministic, non-silent); still no *gate* blocking on it, and no runtime/semantic detector — a classified route can still hide undocumented behavior the report can't see |
...
| Adversarial (4-detector, bidirectional) | ⚠️ restraint skeleton ✅ shipped (§11.4a, `deliver.md:401`); **Blind-Spot ✅ shipped as a report** (§11.4c, `35e1b1c5`); Contradiction vacuous-deferred (§11.4b), False-Promise/Weak-Spec runtime-deferred (§11.4d) | see §11.4 Adoption status (run_d9bd27da + `35e1b1c5`) — the shipped pieces are deterministic/non-silent; building the vacuous/runtime detectors as gates now is the anti-pattern; activation triggers documented |
...
| **P2** | §11.2 behavior-coverage *gate* / §11.4 remaining detectors | 🟡 **unlocks after ③** | Blind-Spot's *report* already shipped (`35e1b1c5`); what remains here is the fail-closed *gate* form + the runtime detectors: behavior-coverage-as-gate needs runtime + `step.entry_ref` at generation; False-Promise/Weak-Spec need runtime. Naturally unlocked once ③'s test-runner harness exists. Building them before is C042 empty-gate over-engineering (§11.4 already ruled this). |
...
### v3 domain + spec-details layer (2026-07-16) — FOUNDATION SHIPPED (blind-spot reverse-coverage report shipped v3.2; equivalence live-wiring + fail-closed detector *gates* still pending — see §11.7)
...
### v3.2 adversarial reverse-coverage + dead-code precision (2026-07-19, incremental)

| Area | Deliverable | Status |
|---|---|---|
| **Blind-Spot reverse-coverage report (§11.2 / §11.4c)** | wire the previously-dead `blind_spot_scan` (full impl + tests, **zero callers**) into a per-package `BLIND-SPOTS.md`: for each risky span (`risk_areas`/`hot_zones` — deterministic facts) flag any owning file no `steps[].file_path`/`business_rules[].anchor` documents | ✅ SHIPPED (`35e1b1c5`, run_d7b78923) — REPORT-ONLY, never a fail-closed gate (the `behavior_coverage` gate stays C042-deferred); deterministic + non-silent (`clean:true` = valid zero-finding outcome) |
| **Dead-code detector false-positives** | fix top-3 FP classes in the code-intel dead-code pass (inheritance/`extends` chains + siblings) | ✅ SHIPPED (`7740d6db`) |

**Honest boundary:** v3.2 ships the *report* side of Blind-Spot (the highest-value 1 of the 4 adversarial detectors) and tightens dead-code precision. It does NOT ship the fail-closed `behavior_coverage` gate, the Contradiction 7-class subset, or the runtime False-Promise/Weak-Spec detectors — those remain deferred with activation triggers (§11.4b/d, §11.7).
...
| **[graphify](https://github.com/Graphify-Labs/graphify)** (Graphify-Labs, YC S26, 90K★, MIT; source-level dive 2026-07-18 — see §24) | Benchmark-methodology + topology-analysis source; positioning contrast. tree-sitter AST → NetworkX → Leiden, 0-LLM graph, EXTRACTED/INFERRED/AMBIGUOUS edges (we ported the confidence+guard, run_2392a203). | **§24 steal-list:** (1) public benchmark w/ dual-judge κ=0.81 on LOCOMO/LongMemEval → closes OT03; (2) `analyze.py` topology menu (god-nodes/surprising-connections/suggested-questions) — pure increment on our existing graph; (3) LOUD capability-gap self-check (#1745/#1689); (4) ERROR-node regex fallback (`sql.py:194`). We reject its positioning: it's a single-point tool (no memory/evolution/semantic-domains), we're an OS. |
...
## 24. Steal-List — graphify (source-level dive, 2026-07-18)

> **Source & provenance.** [`Graphify-Labs/graphify`](https://github.com/Graphify-Labs/graphify) — YC S26, 90K★, ~48 py modules / ~1.5MB. Read at source level (not README): `extract.py` (5135 L), `analyze.py` (740 L), `cluster.py`, `serve.py`, `extractors/sql.py` (295 L), `pyproject.toml`, CHANGELOG. It is a **single-point tool** — "any codebase/docs → queryable knowledge graph" — that ships as a skill into 15+ agents. It is NOT an OS: no cross-session memory, no self-evolution, no business-semantic layer. The parts worth stealing are its **method (benchmark) and its algorithms (topology analysis) and its failure-honesty**, NOT its positioning. Its "understanding" is **topology-level, computed by graph algorithms with zero LLM** — which is exactly its ceiling (it knows `APIRouter` is degree-47, not what it's *for*); our domain/flow layer is where we differentiate. This section records four concrete adoptions ranked by leverage.

### 24.1 Public benchmark methodology — the most-cited gap (directly closes OT03)

**The sting:** graphify publishes `BENCHMARKS.md` with hard numbers on **academic, cross-referenceable** datasets — LOCOMO retrieval **recall@10 = 0.497** (≈10× mem0's 0.048, above BM25's 0.362), LOCOMO QA 45.3%, LongMemEval-S 76%, **0 LLM credits to build the graph** — every system run on one shared model + identical budgets, scored by **a judge blind-validated against a second judge (90.6% agreement, Cohen's κ=0.81)**. Our recall, after the vector leg was torn out (pure-filesystem keyword/FTS5, MOD03), has run on **"感觉够用" with zero quantified evidence**. This is OT03 ("E2E 自动验证链缺失") in its recall/code-intel form.

**What to build (design, not yet a run):**
- A **decoupled, scheduled benchmark subsystem** — same architectural shape as SwarmAI's existing golden-case eval (DEC05/PIT179: eval is system-level, scored against the *deployed* system, never in-pipeline). Immutable question-set snapshot → run-to-run comparison → regression gate.
- **Two suites, mirroring graphify's split:**
  - *recall suite* — known-answer queries against our own workspace corpus (KNOWLEDGE/MEMORY/DDD): does the right section/entry surface in top-K? Metric: **recall@K + MRR**, blind-judged.
  - *code-intel suite* — a fixed agent answers graded architecture questions on a real external repo (graphify uses ERPNext ~1M-LOC + 689 weekly AST checkpoints for a temporal sub-suite) with only our code-intel tool vs a grep/read floor. Metric: **answer accuracy + retrieval recall**.
- **The κ=0.81 discipline is the load-bearing part, not the numbers:** a single LLM judge is not evidence (self-agreement). Adopt **dual-judge with a published agreement coefficient** — this is what makes the number defensible rather than marketing. Folds into §11.5's continuous-eval contract (Siala & Lano precision/recall as golden gates) — 24.1 is the *measurement rig* those thresholds run on.
- **Anti-goal (P6):** the metric serves the outcome. A benchmark number is a regression floor + an honesty forcing-function, never the object. Do not tune recall to a leaderboard; tune it to "the right thing surfaces," and let the number *witness* that.

#### 24.1.1 Resolved design (run_33514fd7, research) — an EXTENSION of the existing eval subsystem, not a new harness

> **Framing correction (Gate-0).** SwarmAI already has a mature eval subsystem — `eval_service.py` (golden_set.yaml + private, in-memory cache), `eval_runner.py` (a SINGLE pinned LLM judge + `file_contains`/`keyword_match`/`trajectory_*`/`canary_pass` scorers + a `recall` category and `recall_chain_probe.py`), `ci_eval_gate.py` (git-bound pre-push gate), and the `eval-scheduled` system job (every-2-week Mon, runs against the DEPLOYED system per DEC05/PIT179). §24.1 is therefore **new scorers + a dual-judge mode + a benchmark suite type riding that runner/gate/schedule** — NOT a parallel subsystem. Building a separate harness would duplicate trigger/judge/history infra (the same reuse-the-existing-machinery lesson that collapsed 24.3 in run_dd13fb03).

**The 5 open points, resolved:**

**(1) Datasets → SELF-BUILD; cite LOCOMO/LongMemEval only as prior art.** Both are *conversational-memory* benchmarks — LOCOMO (10 synthetic multi-session dialogues, CC BY-NC 4.0, native metric QA-F1; recall@K is a *derived* framing mem0/graphify layered on, [arXiv:2402.17753](https://arxiv.org/abs/2402.17753), `github.com/snap-research/locomo`) and LongMemEval (500 questions, `-S` ≈ 40 sessions/115K tok — **not** n=50, MIT, [arXiv:2410.10813](https://arxiv.org/abs/2410.10813), `github.com/xiaowu0162/LongMemEval`). Our recall corpus is a **developer workspace** (markdown KNOWLEDGE/MEMORY/DDD + code-intel) — dialogue-turn retrieval does not transfer (different corpus shape, query distribution, no session structure). No public dataset fits "retrieve from a developer's notes"; the nearest cousins (BEIR, NQ, CodeSearchNet/CoIR) are generic web/code IR. A self-built golden query set is cheap because **we own the ground truth**. ⚠️ **graphify's LOCOMO number is NOT a shared scoreboard** — comparing our workspace-recall figure to it would be different corpus + gold + query types (misleading). [graphify's specific 0.497 / κ=0.81 figures: source unavailable / user-provided.]

**(2) Corpus → reuse the 3 repos ALREADY code-intel-indexed.** No new pick needed: SwarmAI itself (`Projects/SwarmAI/code-intel.json`) + two EXTERNAL repos under `Projects/ai_ready_repo/.artifacts/{ai-ready-mempalace,ai-ready-understand-anything}/.ai-ready/code-intel.json`. The external pair gives a **non-self-referential** code-intel suite. **Pin an immutable `code-intel.json` snapshot per benchmark version** (mirrors the golden-set immutable-snapshot pattern) so a re-index can't move the ground truth under the score.

**(3) Dual-judge κ → validate-on-a-sample ONCE, then single judge at scale.** κ = (p_o − p_e)/(1 − p_e); Landis-Koch: 0.61–0.80 substantial, 0.81–1.0 almost-perfect (so graphify's 0.81 is genuinely strong). Design (cost-conscious, [Zheng et al. arXiv:2306.05685](https://arxiv.org/abs/2306.05685) validation-by-agreement + [Landis & Koch 1977](https://doi.org/10.2307/2529310)): run a SECOND judge on a **validation SAMPLE (100–300 items / ~15–25%)**, compute + **publish κ (with both model IDs, sample size, date)**, then run the cheap single judge on the full set. Re-validate on any judge/prompt change + a rolling per-release spot-check. **Independence lever #1 = a DIFFERENT MODEL FAMILY** for the second judge — two instances of the same model agree *wrongly together* (shared-bias κ certifies nothing). Guard the known LLM-judge biases (self-preference — do not let the judge be the same family that generated the answer; position; verbosity). This adds a `--validate` mode to `eval_runner.py` at ~1× steady-state cost.

**(4) Two suites → different scoring shapes, only one needs the judge.**
- **recall-suite** — known-answer queries over the workspace corpus; gold = the section/file that SHOULD surface. Metric: **recall@K + MRR + nDCG@K**, computed **mechanically** (did the gold section appear in top-K) — *not* LLM-judged, so it's **not circular**. Extends `recall_chain_probe.py` + the existing `recall` category (`eval_runner.py:1707`).
- **code-intel-suite** — a fixed agent answers graded architecture questions on a pinned indexed repo with ONLY the code-intel tool vs a grep/read floor. Metric: **answer accuracy (LLM-judged)** + retrieval recall. **This is where the dual-judge κ matters** (the recall-suite is mechanical and needs no judge).

**(5) Scheduling → hang on the existing rails.** Benchmark runs against the **DEPLOYED** system (DEC05/PIT179) via the `eval-scheduled` job + `/api/eval/canary`, gated by the drift band — **never in-pipeline** (`eval_command_guard`). `ci_eval_gate` stays the pre-push gate; the benchmark is a **new suite type in the same runner**, triggered by CI-post-push / deploy / scheduled. Not a new scheduler.

**Build sizing (PRI06 — runs, not sprints):**
- **Run 1 (`full`)** — recall-suite: golden query set format + `recall@K/MRR/nDCG` mechanical scorer extending `recall_chain_probe`, seeded with ~20–30 known-answer queries. Mechanical, no judge — smallest, highest-certainty.
- **Run 2 (`full`)** — code-intel-suite: fixed-agent harness (code-intel-tool-only vs grep floor) over the 2 pinned external repos + LLM-judged accuracy scorer.
- **Run 3 (`full`)** — dual-judge `--validate` mode: second-family judge on a sample, κ compute + publish, wire the gate on κ≥0.61 (target ≥0.8).
- **Run 4 (`trivial`/`docs`)** — wire suites into `eval-scheduled` + a published benchmark report (numbers + κ + sample + dates).

**Anti-goal reminder (P6):** these numbers are a regression floor + honesty forcing-function, never a leaderboard to tune to. The recall-suite being mechanical (not judge-opinion) is the structural guard against gaming.

### 24.2 Topology-analysis layer — pure increment, data already in the graph

We already emit the graph (`code-intel.json`: `module_edges`, `domains`, `entry_points`, now confidence-labeled per run_2392a203). We do **almost no graph analysis on it.** graphify's `analyze.py` is a ready-made algorithm menu that runs on data we *already have*:

| graphify capability | Mechanism (`analyze.py`) | Our landing |
|---|---|---|
| **god nodes** | degree centrality, ranked; file-level hubs *excluded* so they don't displace real abstractions (`god_nodes`, L100) | "what does everything flow through" — surface top-N most-connected symbols/modules; feed the AGENTS.md "Critical Rules" + spec-details §6 risk. Our `hot_zones` is a weaker cousin — upgrade it with real centrality. |
| **surprising connections** | edge scoring: cross-community + cross-file-type + cross-directory + peripheral→hub + AMBIGUOUS-confidence (`_surprise_score`, L194–263) | cross-domain coupling detector — an edge that jumps two `domains[]` is either a real cross-cutting concern or a leak. Directly feeds spec-details §8 Relations + §7 gaps. |
| **suggested questions** | derived from AMBIGUOUS edges / bridge (high-betweenness) nodes / under-explored god-nodes / isolated nodes (`suggest_questions`, L419–448) | auto-generate "what to clarify" — our AMBIGUOUS/INFERRED edges (run_2392a203) are the exact seed. A confidence≤INFERRED edge between two domains → auto-question "is this dependency real?" for the sign-off human. |
| **communities** | Leiden (graspologic) w/ Louvain fallback, LLM-free labels, >25% communities force-split (`cluster.py`) | **do NOT adopt as-is** — our `domains[]` are *business-semantic* (human/LLM-named: "Chat & Session Lifecycle"), strictly richer than a topology cluster (`community_2`). Use Leiden only as a *cross-check*: if the topology cluster cuts hard across a hand-drawn domain boundary, flag the domain for review. Topology validates semantics; it does not replace it. |

**Why this is cheap:** it's read-only graph math on `code-intel.json` — no new extraction, no LLM, deterministic. New module (e.g. `graph_analysis.py`) consuming the graph, feeding spec-details + AGENTS.md. Slots under §11.2 (blind-spot) and §5 (domain layer) as an analysis pass.

### 24.3 "Capability-gap must be LOUD, never silent" — a coverage self-check

graphify treats **silent under-contribution as a P-class bug**, twice (CHANGELOG #1745, #1689): a `.sql` file whose extractor bailed for a missing optional dep contributed **0 nodes while the graph "built successfully"** — neither the no-extractor warning (#1689) nor the zero-node warning (#1666) fired, so an entire SQL corpus vanished with no trace. Fix: `extract()` surfaces skipped files **grouped by extension, naming the extra that would restore them**. This is the same principle as our §11.1 fail-closed anchor accounting (ratio=1.0, no silent omission) — but extended to a class we don't currently guard: **files the pipeline never even attempted.**

**What to build:** a code-intel **skipped-file ledger** — every file the collector saw but did NOT index (unsupported extension, parser bailed, size cap, sensitive-dir prune) is counted + grouped by reason, emitted as a loud per-run signal (not a debug log). Ties to §11.1: anchor accounting proves "everything we *parsed* is classified-or-reasoned"; this proves "we know what we *didn't* parse." Without it, `classified_ratio` is computed over a silently-shrunk denominator — a confidently-wrong coverage number, exactly the failure mode §11.6 warns about.

### 24.4 ERROR-node layered fallback — parse-fail degrades to regex, never drops

When tree-sitter emits an `ERROR` node (a construct the grammar can't parse), graphify does **not drop the whole block** — it regex-scans the ERROR blob to at least recover object *names* + a `contains` edge, leaving the body opaque (`sql.py:194–212`; the `fb_proc_or_trigger` Firebird path L213 is a full regex-driven fallback extractor for a dialect tree-sitter doesn't cover). Two CHANGELOG lessons ride with it: **(#1854)** an unparseable routine's ERROR node *swallows trailing statements* into its error region — order matters, emit the structural edges (FKs) BEFORE the fragile ones; **(#1910)** PL/pgSQL bodies parse as ERROR and were silently dropped until the regex-name recovery was added.

**What to adopt:** a **three-tier extraction contract** for any language where our tree-sitter coverage is partial — (1) AST-quality when the grammar parses; (2) regex-name-recovery on an ERROR node (keep the node + `contains`, body opaque); (3) the skipped-file ledger (24.3) if even that fails. Never let a single unparseable construct zero-out a file. This is the general form of the R16b/parser-robustness spirit: degrade legibly, account for what you couldn't read. **Design note (honesty):** graphify itself proves this is a *floor, not a solution* — its regex fallback deliberately does NOT scan bodies (loop vars/locals produce junk edges), so recovered objects are name-only islands. The tier is about not-losing-the-node, not about understanding it.

### 24.5 What we do NOT steal (the moat)

graphify stops at "understand one repo, then hand off." It has **no memory, no evolution, no business-semantic domains, and does not consume its own graph** — it nudges *external* agents to query it (strict-mode PreToolUse hook blocks the first raw source read). Our value is exactly downstream of where it stops: the graph is not the deliverable, it is the **raw material fed into the DDD + recall + cultivation + evolution loop** so SwarmAI understands *itself* and acts on it. Adopt graphify's rigor (24.1) and its algorithms (24.2–24.4); reject its positioning. Different species.
