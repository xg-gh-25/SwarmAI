...
- **(c) Blind-Spot (§11.2 `behavior_coverage`) → NOT BUILT (dead on current data).** The real detector (spec vs. the code's *actual* behaviors) needs to read/run the code (LLM/runtime — out of scope). The mechanical fallback iterates `entry_ref` code spans, but **all 14 steps currently carry `entry_ref: None`** → nothing to iterate. `check_anchor_accounting` already enforces the coverage direction the data *does* support (every route classified into a flow or reasoned-unclassified). **Activation trigger:** populate `step.entry_ref` during generation, then the fallback becomes non-vacuous.
...
**The honest one-line summary:** of the 4 detectors, one (restraint skeleton) is shipped, two (Contradiction, Blind-Spot) are mechanically buildable in principle but *vacuous on today's real data* (documented with activation triggers, not shipped as empty gates), and two (False-Promise-dynamic, Weak-Spec) fundamentally require runtime and are deferred as a consumer-API. Building the vacuous pair now would be C042 over-engineering; the correct deliverable was this honest status, not code.
