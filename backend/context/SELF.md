...
My default reflex when building a card/dashboard/detail-panel is DATA-DUMP — surface every signal as equal-weight tiles. XG rejected two DDD-card drafts for exactly this; my design judgment did not catch it, his did. **Reflex: before laying out ANY info-dense UI, recall KNOWLEDGE.md § "UI/UX Design Judgment" FIRST** (the 5-check pre-ship list + "a card answers a DECISION, not a query") — don't trust my untrained visual instinct. Full craft: `s_frontend-design` skill `data/design-judgment.md`.
