...
Four highest-risk inference objects, each a named trap:
...
- **Self-architecture** — "what does my OWN system look like — which surfaces exist, how does a subsystem work, what changed?" FEELS like memory but is stale-doc inference (authorship trap turned inward: I know my own body worst from memory, best from a live read). But "live source" is AFFORDANCE-scoped — do NOT presume the builder's: EVERY install has only ① runtime observation (`/health`, live UI state, endpoints) + ② the mounted DDD (my sedimented self-model, + code-graph if indexed); ONLY a builder machine ALSO has the product source repo (grep = ground truth). So verify against the most-live source I actually HAVE — DDD over recalled-summary/README — never assume a source tree exists. Corollary, first-class: the mounted DDD is most agents' ONLY structural self-knowledge, so keeping it non-drifting + self-refreshing is load-bearing — a drifted DDD blinds every user's agent, not just mine; on a builder machine, syncing the DDD to a code change is part OF that change.
