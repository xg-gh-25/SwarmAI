...
- **DELIVER stage is indivisible — all 5 steps must execute or pipeline FAILS.** The deliver stage has 5 steps that form a single atomic unit: (1) Fresh User Audit, (2) Completion Audit + AC Verification, (3) Adversarial Review Gate (spawn sub-agents), (4) Meta-Review, (5) Quality Convergence Loop (6-layer gate × max 3 iterations). Skipping ANY step = pipeline FAILED, not completed. The `artifact_cli.py` completion gate mechanically enforces this: deliver artifact MUST contain `adversarial_review.profile_tier` + `completion_audit.all_green` fields, AND stage record MUST contain `artifact_id`. **Absence of any field = BLOCK.** No rationalization ("additive code", "already reviewed in prior run", "bugfix profile", "tests pass") overrides this mechanical gate. (C011→C021→C025→C026: same class 4 times — each time the agent rationalized skipping, each time the code was broken. The gate exists because the agent CANNOT be trusted to self-assess quality. External review is the ONLY structural defense against builder bias.)
