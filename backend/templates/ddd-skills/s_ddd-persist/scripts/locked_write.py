...
def locked_read_modify_write(
    ...
    *, dedup: bool = False,
    ...
    try:
        ...
        pass  # DDD-portable: core.memory_guard is a SwarmAI injection-guard — absent on
              # Kiro/Claude Code by design. Degrade to a plain locked write (a DDD's own
              # docs are not the always-injected SwarmAI MEMORY.md, so the guard is n/a).
