...
from _ddd_paths import publish_log_path as _publish_log_path


def build_parser():
    parser = argparse.ArgumentParser(
        description='Log a publish event to the pollinate publish log',
        epilog='Appends one JSON line to <workspace>/.artifacts/pollinate/publish-log.jsonl'
    )
    ...
    parser.add_argument('--log-path', default=None,
                        help='Path to log file (default: <workspace>/.artifacts/pollinate/publish-log.jsonl)')
    return parser
...
def log_publish(topic, channel, format_type, url=None, log_path=None):
    ...
    if log_path is None:
        log_path = str(_publish_log_path())
