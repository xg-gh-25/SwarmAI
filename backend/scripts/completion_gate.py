...
def completion_surface_verdict(
    ...
    # Source work but nothing committed → the hand-commit / no-commit bypass.
    # (Also fires when commits exist but none intersect THIS run's source —
    #  i.e. only sibling-session commits — because then this run's OWN source
    #  was never committed via run-commit.)
    run_scoped_committed = bool(committed_files) and _tail_match(committed_files, set(source_files))
