...
def entry_dedup_keys(line: str) -> tuple[str | None, str | None]:
    ...
    memory_extractor (R3-C). Mirrors the keys distillation has used inline
    since its dedup was added:
...
def locked_read_modify_write(
    ...
    *, dedup: bool = False, reindex_memory: bool = False,
    ...
    Args:
        ...
        reindex_memory: INERT — accepted and ignored. The in-prompt MEMORY index
            was deleted (live MEMORY.md is full-injected; recall scans the body
            with FTS5+BM25), so there is no index to rebuild after a write and
            its generator functions no longer exist. The parameter is retained
            only so existing call sites keep working unchanged; passing True has
            no effect. See the write-back site below for the full rationale and
            for the orphan-block strip that replaced it.
    ...
    try:
        ...
        #
        # STRIP any orphan `## Memory Index` block on write-back
        # for MEMORY.md. The block is regenerated by nothing (inject_index_into_memory
        # has zero live callers), stripped on every recall READ (extract_body_without_index),
        # and is ~200 stale lines injected in full into the system prompt. A one-time
        # data edit could NOT durably remove it: this read-modify-write path preserves
        # whatever it READS, so the live 24/7 daemon's next section write re-carried the
        # block forward (verified: block reappeared minutes after a flock-safe manual
        # removal). Stripping here at the SINGLE write chokepoint kills it permanently —
        # the block dies the next time ANY writer touches MEMORY.md and never returns.
        if file_path.name == "MEMORY.md":
            try:
                from core.memory_index import extract_body_without_index
                stripped = extract_body_without_index(new_content)
                if stripped and stripped != new_content:
                    new_content = stripped if stripped.endswith("\n") else stripped + "\n"
            except Exception:  # noqa: BLE001 — strip is best-effort; never block a write
                pass
