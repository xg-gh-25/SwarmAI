...
python3 skills/weekly-revenue-report/midweek_generator.py              # 只生成，不发送
```
...
| RFHC Mid-Week 生成 | `midweek-rfhc-generate` | 周三 10:00 | 周三 18:00 | `midweek_generator.py` → 只生成，不发送 |
...
**⚠️ 邮件发送已禁用（2026-04-13）** — `sender.py` 已删除。报告只生成不发送。需要发送时手动操作。
...
修改 `templates/`、`data.py`、`generator.py` 等核心文件后，**必须**运行 eval 验证：
...
| **v4** | **2026-03-31** | **CEO 简易版独立模板 `ceo_lite.py`**：单页白底、KPI 卡片 + line chart（SVG→PNG→base64 `<img>`，Outlook Desktop 兼容）、全 inline CSS。GM WoW `abs<1%` 去掉正负号。**midweek_generator.py**：RFHC 周四独立报告（自然周 Mon-Sun）。`cairosvg` 依赖新增 |
| **v4.1** | **2026-04-13** | **sender.py 删除** — 邮件发送功能移除，报告只生成不发送。Eval #4 (sender dry-run) 同步删除 |
