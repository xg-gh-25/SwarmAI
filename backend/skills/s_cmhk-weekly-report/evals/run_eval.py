...
Usage:
  ...
  # 跑 sender dry-run 验证（eval #4，需要 AWS 权限）
  python3 run_eval.py --eval-ids 4 --live
...
def eval_4_sender_dryrun(output_dir: str, live: bool = False) -> dict:
    """Eval #4: sender.py --dry-run"""
    results = []

    if not live:
        # Can't run sender without --live
        for exp_text in [
            "sender.py 被调用，参数包含 --dry-run",
            "输出中列出 arobchu 的发送计划，模式为 CEO inline",
            "输出中列出至少 5 位 GM 收件人",
            "没有实际发送邮件（dry-run 模式）",
            "输出中包含邮件 Subject 格式",
            "没有 Python traceback 或 error",
        ]:
            results.append({"text": exp_text, "passed": None, "evidence": "Skipped (not --live mode, sender needs AWS creds)"})
        return _summarize(4, "sender.py --dry-run", results)

    cmd = f"cd {SKILL_DIR} && python3 sender.py --dry-run"
    proc = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=60)
    output = proc.stdout + proc.stderr

    results.append({
        "text": "sender.py 被调用，参数包含 --dry-run",
        "passed": proc.returncode == 0,
        "evidence": f"Exit code: {proc.returncode}",
    })

    # arobchu CEO inline
    has_arobchu = "arobchu" in output
    has_ceo_inline = "CEO inline" in output
    results.append({
        "text": "输出中列出 arobchu 的发送计划，模式为 CEO inline（正文内嵌，无附件）",
        "passed": has_arobchu and has_ceo_inline,
        "evidence": f"arobchu: {has_arobchu}, CEO inline: {has_ceo_inline}",
    })

    # At least 5 GM recipients
    gm_aliases = ["kenshen", "zhangaz", "mzji", "tiafeng", "akchan", "danffer", "chrisso", "gufan", "ligxi"]
    gm_found = [a for a in gm_aliases if a in output]
    results.append({
        "text": "输出中列出至少 5 位 GM 收件人（如 kenshen, zhangaz, mzji 等），模式为附件",
        "passed": len(gm_found) >= 5,
        "evidence": f"Found {len(gm_found)}/{len(gm_aliases)}: {gm_found}",
    })

    # Dry run mode (no actual send)
    has_dryrun = "dry_run" in output or "Dry run" in output or "dry run" in output.lower()
    results.append({
        "text": "没有实际发送邮件（dry-run 模式不触发 Lambda invoke）",
        "passed": has_dryrun,
        "evidence": f"Dry-run indicator found: {has_dryrun}",
    })

    # Subject format
    subject_match = bool(re.search(r"CMHK Weekly Usage & Revenue Report.*W\d+", output))
    results.append({
        "text": "输出中包含邮件 Subject 格式：CMHK Weekly Usage & Revenue Report — W{week}",
        "passed": subject_match,
        "evidence": f"Subject pattern found: {subject_match}",
    })
    ...
    results.append({
        "text": "没有 Python traceback 或 error 出现在执行过程中",
        "passed": not has_error,
        "evidence": "Clean execution" if not has_error else output[-300:],
    })
