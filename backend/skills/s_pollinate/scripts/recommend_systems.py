...
CARDS = Path(__file__).resolve().parent.parent.parent / "s_frontend-design" / "data" / "slide_bold_previews"
THUMB_REL = "Knowledge/assets/deck-styles"   # workspace-relative (served by /api/workspace/file/raw)
# Chat markdown needs an ABSOLUTE url (ContentBlockRenderer passes no basePath, so a
# relative ![](Knowledge/...) won't resolve; resolveImageSrc passes http(s):// through).
DAEMON_PORT = os.environ.get("SWARMAI_PORT", "18321")
THUMB_URL_PREFIX = f"http://localhost:{DAEMON_PORT}/api/workspace/file/raw?path="
...
def main():
    ...
    for sc, c, why in page:
        out.append({
            ...
            "tagline": c["tagline"],
            "thumbnail": f"{THUMB_REL}/{c['slug']}.png",
            "thumbnail_url": f"{THUMB_URL_PREFIX}{THUMB_REL}/{c['slug']}.png",  # use THIS in chat markdown
            "why": why,
        })
    result = {
