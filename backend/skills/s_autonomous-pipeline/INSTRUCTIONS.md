...
1. Update run status to "completed" — the report is generated FOR you:
   ...
   **Why completion comes FIRST (do not swap these back):** `completed_at` is
   ...
   The `>=500 bytes` gate below is already satisfied by the report `reflect.md`
   step 10 produced, so completing first does not trip it.

   **Scope of the auto-refresh (do not over-read it):** it fires only for reports
   carrying `report_autogenerated`. A hand-written report is deliberately skipped,
   and so is one predating that flag (62 of 545 real completed runs) — those keep
   whatever `Duration:` they had and need an explicit `run-report --force`. The
   refresh is also best-effort: a render failure records `report_refresh_failed`
   and never undoes a completion that is already persisted.
