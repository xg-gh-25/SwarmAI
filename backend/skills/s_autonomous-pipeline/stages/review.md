...
| **How** | RP1-RP80 checklist, integration trace, DDD conformance | Free-form attack with specialist prompts |
...
requires: integration trace (real wiring check), runtime patterns RP1-RP80
...
1. Code Quality Agent (review-agents/code-quality.md)
   → TECH.md conformance, integration trace, replace/move parity,
     runtime patterns RP1-RP80, depth & seam analysis
...
- 2: Security Scan
- 3: Integration Trace
- 6: Runtime Pattern Checklist (RP1-RP80) — includes RP35 (pool contention), RP36 (fix-enables-regression), RP37 (process tree lifetime)
...
**BLOCKING: Read `backend/skills/s_autonomous-pipeline/REVIEW_PATTERNS.md` and apply RP1-RP80.**
...
| "Runtime patterns RP1-RP80 — most don't apply" | Check each and mark N/A explicitly. LL08: `asyncio.get_event_loop()` (deprecated, RP19) and `date('now')` UTC mismatch (RP18) both passed pipeline because "didn't seem to apply." They applied. | LL08 |
