...
### Pipeline Boundary: PUSH-READY = Pipeline Done

**The pipeline's job ends at PUSH-READY.** Push to remote and CI verification
are USER-INITIATED actions, not pipeline steps.

```
Pipeline scope:         code quality達標 → PUSH-READY → REFLECT → COMPLETE
User scope (after):     git push → CI green → PR (optional)
```

**Why this boundary exists:**
- Push is a deployment decision (user controls when remote gets updated)
...
- STEERING.md "Post-Push CI Ownership" governs push+CI — that's a per-push
  rule, not a per-pipeline rule
