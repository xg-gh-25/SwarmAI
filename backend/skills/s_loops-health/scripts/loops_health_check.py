...
# The orphan `## Memory Index` block was removed from live
# MEMORY.md (regenerated by nothing since the no-index architecture; recall strips
# it on read anyway). Dropped from the required-anchor set so the integrity gate
# does not false-fail on its absence — the exact "permanently dead auto-commit"
# class the _ok_to_commit comment (~:808) warns about (a removed section left in
# `required` makes the gate return False unconditionally).
MEMORY_SECTIONS = [
    "## Principles", "## Corrections", "## Decisions",
...
PROBE_REGISTRY = [
    ...
    # The commit-context probe now anchors on ## Decisions
...
class SelfLoopsHealthEngine:
    ...
    def _validate_probes(self):
        ...
        skill's own probes are validated before the skill validates the system(mirrors the Q0 gate added to s_chat-brain-check)."""
        drifted = []
        cache = {}
    ...
    def _check_memory(self):
        ...
        # Live 7-type entries are "- [<TYPE><nn>] <title> | ..." — a bracketed type
        # code plus a zero-padded ordinal;
    ...
    def _fix_commit_context(self) -> bool:
        ...
        if mem.exists():
            ...
            # exist today.
            # Dropped "## Memory Index" (orphan block removed;
            # regenerated by nothing) — keep the stable structural anchors.
            required = ["## Decisions", "## Open Threads"]
