...
1. Detect what skill/package was worked on → 2. Analyze session for friction & feedback → 3. Gather evidence → 4. Generate structured report → 5. Save report → 6. Offer to implement improvements
...
**Default location:** `~/shared/.skill-feedback/skill-feedback-{name}-{YYYYMMDD-HHMM}.md`

Create the directory if it doesn't exist:

```bash
mkdir -p ~/shared/.skill-feedback
```
...
```bash
mv ~/shared/.skill-feedback/skill-feedback-{name}-{date}.md \
   ~/shared/.skill-feedback/completed/
```
