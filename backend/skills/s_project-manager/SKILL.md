...
> structure (DDD_SPEC_VERSION 1.0) — see "The Canonical DDD Structure" below. They
> are what CREATE provisions first; ③–⑥ accrete as the project grows.
...
|---|---------|-----------|------------------|:---:|
| ① | **IDENTITY & MANIFEST** | OWN | project name/dir, `bindings.yaml` header, (for AIM pkgs: Config·aim.json·AGENTS.md·.crux_template.md) | ✅ dir + name |
| ② | **KNOWLEDGE** | OWN | **the 4 docs (PRODUCT/TECH/IMPROVEMENT/PROJECT.md) + `Knowledge/`** — 冷启动 + judgment BORN here as prose | ✅ the 4 docs |
| ③ | **GATES** (the moat) | OWN | `gates/<gate>.py|sh` + tests + `context/includes/*denied*.json` — executable terminus of matured judgment | ⬜ added when judgment matures (养成 ladder) |
| ④ | **CAPABILITIES** | OWN | `skills/`, `agents/*.agent-spec.json`, distributed portable capabilities (s_ai-ready-repo, s_persist, s_project-manager) | ⬜ added when a capability is bound |
| ⑤ | **DELIVERY CONTRACT** | GOVERN | **`bindings.yaml`** — full delivery 全貌 per repo: build_system · version_set · branch · deploy_pipeline (ref) · review_path · refresh_policy · auto_send (all DATA) | ✅ when a repo is bound (see BIND) |
| ⑥ | **CODE-INTEL REFRESHER** | GOVERN | a self-contained mechanism that REGENERATES the code-intel projection from code (ship the refresher, not the projection) | ⬜ dev-consumer profile only |

**NOT a DDD member (derived/physical zone):** `code-intel.json` (machine projection —
regenerated by ⑥, gitignored, never a `bindings.yaml` field, never PR-flows-back),
`code_intel.db` (local query engine), the product source repos (GOVERNed via ⑤, never
contained), the deploy pipeline (runs on Amazon infra, referenced in ⑤, never executed).

**What CREATE provisions vs what accretes:** a fresh project starts with ①+② (dir +
the 4 docs). ③④⑤⑥ are **additive** — ⑤ appears the moment a repo is bound (BIND, below);
③④⑥ accrete as the project grows judgment/capabilities. A **no-repo project** legitimately
has only ①+② (no `bindings.yaml`, no ⑤/⑥) — the 4 docs ARE the whole deliverable. A
**GitHub project** gets ⑤ with `remote_kind: github-pr` (no brazil/versionset). This is
why the 6 sections are a superset, not a checklist every project must fill.
