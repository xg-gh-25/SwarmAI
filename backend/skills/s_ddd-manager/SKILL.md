...
> 2026-07-12, option A):** CREATE materializes the **skeleton** of ①③④⑥ (the dirs +
> ① identity manifests + a purpose-README per section) so the standard is CONCRETE
> in SwarmWS — SwarmAI-maintenance follows it, and AIM export is low-variance. The
> section **content** (real gates, skills, agent-specs, a live refresher) still
> **ACCRETES** as the project grows. "Provisioned by CREATE?" below means the
> *skeleton*, not the content.

| # | Section | OWN/GOVERN | Concrete members | Skeleton provisioned by CREATE? |
|---|---------|-----------|------------------|:---:|
...
Also sanctioned non-section dirs at project root: `assets/`, `templates/`, `.artifacts/`.

**What CREATE provisions vs what accretes:** a fresh project's SKELETON is ①②③④⑥
(dir + `.project.json` + 3 identity manifests + the 4 docs + `Knowledge/` + `gates/`
`skills/` `agents/` `agent-sops/` `REFRESHER.md`, each dir carrying a purpose-README).
**Only ⑤ waits** — `bindings.yaml` appears the moment a repo is BOUND (BIND, below),
because repo shape isn't known at create. Section **content** then accretes: gates as
judgment matures, skills/agent-specs as capabilities are bound, a live refresher once a
repo is bound. A **no-repo project** keeps the ①-④+⑥ skeleton but ⑥ stays a no-op marker
and never gets ⑤ — the 4 docs + skeleton are the whole deliverable. A **GitHub project**
gets ⑤ on BIND with `remote_kind: github-pr` (no brazil/versionset). The skeleton is a
concrete standard; the six sections are a superset whose *content* is filled on demand.
