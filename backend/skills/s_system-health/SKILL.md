...
**Why?** Quickly diagnose if your system is struggling and get actionable fixes without digging through Activity Monitor or htop.
...
> - macOS: If `pmset` returns nothing, you're on a desktop Mac, skip battery. If `vm_stat` fails, fall back to `top -l 1`.
> - Linux: If `/sys/class/power_supply/BAT0/` doesn't exist, skip battery (desktop or VM). If `free` is missing (unlikely), fall back to `cat /proc/meminfo`.
...
### Example 2: macOS Low Battery + High RAM
...
### Example 3: macOS Stuck Process

```
🔋 Battery: 45% (charging)
💾 RAM: 14GB / 24GB (58%) — healthy
⚡ CPU: High load
💿 Disk: 200GB / 500GB (40%) — healthy
...
### Example 4: Healthy Linux Server/Desktop

```
💾 RAM: 5.2GB / 16GB (32%) — healthy
⚡ CPU: Light load (load avg: 0.45, 0.38, 0.31 on 4 cores)
💿 Disk: 34GB / 200GB (17%) — healthy

Top memory: java (820MB), code (650MB), chrome (480MB)
Top CPU: java (4.2%), code (1.8%), chrome (1.1%)

✅ System looks healthy — no action needed
(install speedtest-cli for network speed test)
```

### Example 5: Linux Under Pressure
...
⚠️ Disk at 93% — consider cleaning up
...
- [ ] Percentages calculated correctly (used/total x 100)
...
- **Edge cases verified**: Desktop Mac (no battery), Linux VM (no battery, no speedtest), fresh boot (low usage), compile job (high CPU spike), Linux high buff/cache
...
df -h /
cat /proc/loadavg
which speedtest-cli && speedtest-cli --simple || echo "No speedtest-cli"
```
