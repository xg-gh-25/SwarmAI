...
def classify_git_status_paths(
    ...
    """
    root = Path(repo_root)
    if not porcelain_paths:
        return []
    # run_4de279ca: batch the check-ignore into ONE subprocess for this tree
    # (was N per-file needs_human_review calls → N check-ignore subprocesses).
    # Absolutize each rel against ITS OWN repo_root (the F1 contract) BEFORE the
    # batch classifier, which re-derives the owning tree + relativizes internally.
    abs_by_rel = {rel: str((root / rel).resolve()) for rel in porcelain_paths}
    verdicts = needs_human_review_batch(
        list(abs_by_rel.values()), "written", swarmws_root=swarmws_root
    )
    # Preserve input order; map each rel back via its absolute key.
