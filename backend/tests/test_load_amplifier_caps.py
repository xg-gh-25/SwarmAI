...
# ── AC6: Per-turn tool-loop budget (single escalation ladder) ───────


class TestAC6ToolLoopBudget:
    """G3: a turn exceeding a tool-call budget escalates the EXISTING ladder."""

    def test_tool_count_budget_escalates(self):
        """FORCED: exceeding the per-turn tool count escalates via _escalation."""
        from core.compaction_guard import _TURN_TOOL_COUNT_BUDGET

        guard = CompactionGuard()
        # Record diverse calls (so consecutive/diversity detectors stay quiet)
        # exceeding the per-turn count budget.
        for i in range(_TURN_TOOL_COUNT_BUDGET + 2):
            guard.record_tool_call(f"Tool{i}", {"arg": i})
        level = guard.check()
        assert level != EscalationLevel.MONITORING

    def test_budget_uses_single_escalation_ladder(self):
        """The budget must NOT create a parallel kill path (PIT10).

        It steps the same self._escalation field, so the level returned by
        check() equals the stored escalation level.
        """
        from core.compaction_guard import _TURN_TOOL_COUNT_BUDGET
        ...
        for i in range(_TURN_TOOL_COUNT_BUDGET + 2):
            guard.record_tool_call(f"Tool{i}", {"arg": i})
        level = guard.check()
        assert level == guard._escalation

    def test_per_turn_count_resets_on_reset(self):
        """reset() (new user turn) must clear the per-turn tool counter.

        _tool_records is subprocess-cumulative (preserved by reset), so the
        budget MUST use a separate per-turn counter that reset() clears —
        otherwise a healthy multi-turn session false-triggers.
        """
        from core.compaction_guard import _TURN_TOOL_COUNT_BUDGET
        ...
        for i in range(_TURN_TOOL_COUNT_BUDGET + 2):
            guard.record_tool_call(f"Tool{i}", {"arg": i})
        guard.reset()  # new user turn
        # After reset, the per-turn counter is clear → a few calls don't trigger.
        guard.record_tool_call("ToolA", {"a": 1})
