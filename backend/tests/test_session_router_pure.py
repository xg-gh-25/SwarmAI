...
class TestIsWarmReuse:
    """_is_warm_reuse — pure predicate deciding whether a turn can REUSE the
    live subprocess AND its cached spawn-options (skip the expensive
    build_system_prompt). run_f8c3ddd4 refactor: system_prompt assembly is a
    spawn-time-once concern; a warm-reuse turn must NOT rebuild it. The predicate
    is the single source both the build-skip gate and the UI-prefix decision read.

    Warm-reuse requires ALL: state==IDLE, a live client, last turn clean, AND a
    cached spawn-options object to reuse. Missing ANY → cold/rebuild path.
    """

    def _call(self, state="IDLE", has_client=True, last_turn_clean=True,
              has_cached_options=True):
        from core.session_router import _is_warm_reuse
        from core.session_unit import SessionState
        state_map = {
            "IDLE": SessionState.IDLE, "COLD": SessionState.COLD,
            "STREAMING": SessionState.STREAMING, "DEAD": SessionState.DEAD,
            "WAITING_INPUT": SessionState.WAITING_INPUT,
        }
        return _is_warm_reuse(
            state=state_map[state], has_client=has_client,
            last_turn_clean=last_turn_clean, has_cached_options=has_cached_options,
        )

    def test_all_conditions_met_is_warm(self):
        """IDLE + live client + clean + cached options → warm reuse (skip build)."""
        assert self._call() is True

    def test_cold_is_not_warm(self):
        """COLD state → must build (turn-1 / respawn). NEVER reuse."""
        assert self._call(state="COLD") is False

    def test_dead_client_is_not_warm(self):
        """IDLE but client gone (CLI exited after error_max_turns) → must respawn."""
        assert self._call(has_client=False) is False

    def test_not_clean_is_not_warm(self):
        """last_turn_clean=False (poison-guard recycle territory) → must rebuild,
        NEVER reuse a stale cached prompt. This is the load-bearing safety gate."""
        assert self._call(last_turn_clean=False) is False

    def test_no_cached_options_is_not_warm(self):
        """No cached spawn-options yet (first turn before any spawn) → must build."""
        assert self._call(has_cached_options=False) is False

    def test_streaming_is_not_warm(self):
        """A protected active state is never a warm-reuse entry."""
        assert self._call(state="STREAMING") is False

    # ── Layer-4 cross-boundary E2E (cross_boundary=true, multi-subsystem) ──
    # The seam: a REAL SessionUnit's actual attributes (state / _client /
    # _last_turn_clean / _spawn_options) flow into _is_warm_reuse, whose output
    # gates whether run_conversation rebuilds the ~91K prompt. These tests drive
    # the REAL SessionUnit + REAL SessionState + REAL predicate (no mock of the
    # thing-under-change), and mutation-verify below.

    def test_real_unit_turn1_is_cold_must_build(self):
        """E2E: a fresh REAL SessionUnit (turn-1: COLD, no client, no cached
        options) → NOT warm → run_conversation MUST build. This is the path that
        keeps recall working (turn-1 is always cold)."""
        from core.session_unit import SessionUnit
        u = SessionUnit(session_id="l4-cold", agent_id="default")
        from core.session_router import _is_warm_reuse
        assert _is_warm_reuse(
            state=u.state, has_client=u._client is not None,
            last_turn_clean=u._last_turn_clean,
            has_cached_options=u._spawn_options is not None,
        ) is False, "a fresh unit must take the BUILD path (recall depends on it)"

    def test_real_unit_warm_after_spawn_skips_build(self):
        """E2E: a REAL SessionUnit driven into the warm-reuse state (IDLE + live
        client + clean + cached options) → warm → run_conversation SKIPS build.
        Mutation-verify: flip ANY one real attribute → the decision flips to
        BUILD (proves the gate is non-vacuous and reads every input)."""
        from unittest.mock import MagicMock
        from core.session_unit import SessionUnit, SessionState
        from core.session_router import _is_warm_reuse
        u = SessionUnit(session_id="l4-warm", agent_id="default")
        u._transition(SessionState.IDLE)          # real transition COLD→IDLE
        u._client = MagicMock()                    # live client
        u._last_turn_clean = True                  # clean last turn
        u._spawn_options = MagicMock()             # a prior spawn cached options

        def decide(unit):
            return _is_warm_reuse(
                state=unit.state, has_client=unit._client is not None,
                last_turn_clean=unit._last_turn_clean,
                has_cached_options=unit._spawn_options is not None,
            )

        assert decide(u) is True, "warm state on a REAL unit must skip build"

        # MUTATION-VERIFY: each real attribute is load-bearing — flipping any one
        # must send the decision to BUILD (RED if the gate ignored that input).
        u._last_turn_clean = False
        assert decide(u) is False, "not-clean MUST force build (poison-recycle safety)"
        u._last_turn_clean = True; u._client = None
        assert decide(u) is False, "dead client MUST force respawn/build"
        u._client = MagicMock(); u._spawn_options = None
        assert decide(u) is False, "no cached options MUST force build"
        u._spawn_options = MagicMock(); u._transition(SessionState.STREAMING)
        assert decide(u) is False, "non-IDLE MUST force build"
