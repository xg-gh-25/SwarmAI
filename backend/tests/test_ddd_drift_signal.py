...
# ─── AC3: briefing System-health drift line (present iff drift_count > 0) ──────

class TestBriefingDriftLine:
    def _run(self, monkeypatch, drift):
        import core.proactive_intelligence as pi
        # _get_ddd_drift_line does a function-local `from core.ddd_drift_signal import
        # get_semantic_drift`, so patch it at the SOURCE module (not on pi).
        import core.ddd_drift_signal as ds
        monkeypatch.setattr(ds, "get_semantic_drift", lambda root: drift)
        import tempfile
        with tempfile.TemporaryDirectory() as d:
            return pi._get_ddd_drift_line(d)

    def test_line_present_when_drift(self, monkeypatch):
        drift = {"report_date": "2026-07-20", "drift_count": 2, "at_risk_cases": [],
                 "findings": [{"project": "CMHK_SalesIntel", "docs": ["PROJECT.md"], "title": "t", "detail": "d"},
                              {"project": "PhysicalAI", "docs": ["PRODUCT.md"], "title": "t2", "detail": "d2"}]}
        lines = self._run(monkeypatch, drift)
        drift_lines = [l for l in lines if "[ddd-drift]" in l]
        assert len(drift_lines) == 1, lines
        assert "2 finding(s)" in drift_lines[0]
        assert "2 project(s)" in drift_lines[0]

    def test_no_line_when_clean(self, monkeypatch):
        drift = {"report_date": "2026-07-18", "drift_count": 0, "findings": [], "at_risk_cases": []}
        lines = self._run(monkeypatch, drift)
        assert [l for l in lines if "[ddd-drift]" in l] == []

    def test_drift_line_never_persists_a_todo(self, monkeypatch):
        # Gate-1 constraint: the drift line must be a plain append, NEVER routed through
        # _create_health_todo (which writes a Radar todo to data.db = persisted state).
        import core.proactive_intelligence as pi
        called = {"n": 0}
        monkeypatch.setattr(pi, "_create_health_todo",
                            lambda *a, **k: called.__setitem__("n", called["n"] + 1), raising=False)
        drift = {"report_date": "2026-07-20", "drift_count": 3, "at_risk_cases": [],
                 "findings": [{"project": "X", "docs": ["TECH.md"], "title": "t", "detail": "d"}]}
        self._run(monkeypatch, drift)
        assert called["n"] == 0, "drift line must not create a Radar todo (persisted state)"
