...
class TestMultiDayRunsShowTheDate:
    """A time-of-day-only column makes a multi-day run read as out-of-order.

    Caught on this run's OWN report: it spans 2026-09-07 to 09-11, so a stage
    published 05:44 on a later day sorted correctly yet displayed above one at
    09:32 — indistinguishable from a broken sort. The rows are right; the
    rendering hid the day.
    """

    def _seed(self, ws, run_id, stamps):
        run_dir = _write_run(ws, "P", run_id, {
            "id": run_id, "project": "P", "profile": "research",
            "requirement": "multi-day", "status": "completed",
            "created_at": stamps[0][1], "updated_at": stamps[-1][1],
            "completed_at": stamps[-1][1], "report_autogenerated": True,
            "stages": [
                _stage(n, f"art_{i}", token_cost=10)
                for i, (n, _) in enumerate(stamps)
            ],
            "taste_decisions": [],
        })
        _write_manifest(ws, "P", [
            _artifact(f"art_{i}", t) for i, (_, t) in enumerate(stamps)
        ])
        return run_dir

    def test_multiday_run_renders_the_date(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SWARM_WORKSPACE", str(tmp_path))
        from scripts.artifact_cli import cmd_run_report, ArtifactRegistry, _get_workspace

        # think publishes EARLIER in the day but on a LATER date — the shape that
        # made a correct sort look broken.
        run_dir = self._seed(tmp_path, "run_md", [
            ("evaluate", "2026-09-07T09:32:00+00:00"),
            ("think", "2026-09-11T05:44:00+00:00"),
        ])
        cmd_run_report(
            SimpleNamespace(project="P", run_id="run_md"),
            ArtifactRegistry(_get_workspace()),
        )
        block = (run_dir / "REPORT.md").read_text(encoding="utf-8")
        block = block[block.find("4.1"):]
        row = next(l for l in block.splitlines() if l.startswith("| think"))
        assert "09-11" in row, (
            "a multi-day run showed only the time of day, so a later-date row "
            f"reads as out-of-order against an earlier one:\n{block[:500]}"
        )

    def test_single_day_run_keeps_the_compact_column(self, tmp_path, monkeypatch):
        """The date must appear ONLY when it is needed."""
        monkeypatch.setenv("SWARM_WORKSPACE", str(tmp_path))
        from scripts.artifact_cli import cmd_run_report, ArtifactRegistry, _get_workspace

        run_dir = self._seed(tmp_path, "run_sd", [
            ("evaluate", "2026-09-07T09:00:00+00:00"),
            ("think", "2026-09-07T09:45:00+00:00"),
        ])
        cmd_run_report(
            SimpleNamespace(project="P", run_id="run_sd"),
            ArtifactRegistry(_get_workspace()),
        )
        block = (run_dir / "REPORT.md").read_text(encoding="utf-8")
        block = block[block.find("4.1"):]
        row = next(l for l in block.splitlines() if l.startswith("| think"))
        assert "09-07" not in row, (
            f"a single-day run does not need the date column:\n{row!r}"
        )
        assert "09:45:00" in row, f"the time must still render:\n{row!r}"
