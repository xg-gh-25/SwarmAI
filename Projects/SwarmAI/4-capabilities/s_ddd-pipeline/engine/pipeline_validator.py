...
# ---------------------------------------------------------------------------
# Portable DDD engine: NO SwarmAI `jobs` package on Kiro/Claude Code, and no hardcoded
# ~/.swarm-ai. Resolve the audit-marker dir from the workspace ($SWARM_WORKSPACE / cwd).
# The Gate-2 agent-tool audit is an ADVISORY depth check (degrades to WARN if the marker
# dir is absent), so this location never blocks a run.
AGENT_AUDIT_DIR = (
    Path(os.environ.get("SWARM_WORKSPACE") or os.getcwd())
    / ".artifacts" / "state" / "pipeline_agent_audit"
)
...
def _get_workspace() -> Path:
    # Portable: no SwarmAI `config` dep — honor $SWARM_WORKSPACE else cwd.
    import os
...
def main() -> None:
    ...
    if args.command == "check":
        ...
        adversarial_health = {}
        try:
            from adversarial_meta import check_adversarial_health  # optional, off-path
