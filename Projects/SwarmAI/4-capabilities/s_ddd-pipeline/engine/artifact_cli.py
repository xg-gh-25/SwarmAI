...
# DDD-native engine: all modules are co-located siblings in this engine/ dir
# (copied + decoupled from SwarmAI's backend/scripts + backend/core). Add THIS
# dir to path so sibling imports resolve on any runtime (Kiro/Claude Code), with
# NO dependency on a SwarmAI `core` package being importable.
sys.path.insert(0, str(Path(__file__).resolve().parent))

from artifact_registry import ArtifactRegistry
from pipeline_profiles import get_profile_stages
...
def _get_workspace() -> Path:
    """Resolve the workspace root that contains Projects/<ddd>/.

    Portable (no SwarmAI `config` dependency): honor $SWARM_WORKSPACE if set,
    else default to the current working directory. On Kiro/Claude Code the agent
    runs inside the DDD's workspace, so cwd is the right root; SwarmAI sets
    $SWARM_WORKSPACE explicitly.
    """
    import os
    ws = os.environ.get("SWARM_WORKSPACE") or os.getcwd()
...
def cmd_run_cultivate(args, reg: ArtifactRegistry) -> None:
    ...
    # Run cultivation
    from ddd_cultivation import cultivate_from_reflect
...
def cmd_ddd_health(args, reg) -> None:
    ...
    try:
        from ddd_health import compute_section_health  # optional module
    except ImportError:
        print(json.dumps({"error": "ddd-health analytics not bundled in the "
                          "portable DDD engine (off the run/reflect path)"}))
        return
...
def cmd_ddd_retire(args, reg) -> None:
    ...
    """
    try:
        from ddd_entry_lifecycle import retire_entry, RetireError, MEMORY_EVERGREEN_SECTIONS  # optional
    except ImportError:
        print(json.dumps({"error": "entry-retirement not bundled in the portable "
                          "DDD engine (off the run/reflect path)"}), file=sys.stderr)
        sys.exit(1)
...
def cmd_ddd_noise(args, reg) -> None:
    ...
    try:
        from ddd_entry_lifecycle import (  # optional module
            parse_entries, compute_reclaimable_noise, NOISE_FAIL_THRESHOLD,
            MEMORY_EVERGREEN_SECTIONS,
        )
    except ImportError:
        print("PASS (noise gate not bundled in the portable DDD engine — "
              "off the run/reflect path)")
        return
    ...
    if getattr(args, "doc", None):
        ...
        if proj_dir.is_dir():
            # Canonical layout is Projects/<name>/2-understanding/IMPROVEMENT.md;
            # the flat Projects/<name>/IMPROVEMENT.md is the legacy fallback. Union
            # both so the noise gate never silently skips projects (root cause of
            # the 2026-05→08 cultivation stall was exactly this flat-path blind spot).
            seen_imp: set = set()
            for imp in sorted(proj_dir.glob("*/2-understanding/IMPROVEMENT.md")) + \
                      sorted(proj_dir.glob("*/IMPROVEMENT.md")):
                # Dedup per-project: prefer 2-understanding/, skip flat dup if the
                # canonical one for the same project already added.
                proj_key = imp.parent.name if imp.parent.name != "2-understanding" else imp.parent.parent.name
                if proj_key in seen_imp:
                    continue
                seen_imp.add(proj_key)
