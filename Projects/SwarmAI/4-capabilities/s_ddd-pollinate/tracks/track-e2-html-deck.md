...
view in a browser / on the web (e.g. GitHub Pages via publish_to_pages.py), or when you
want the frontend-slides bold aesthetic. Both can co-exist in one run.
...
2. **Present them inline** — for each recommendation, emit a standard markdown image
   ...
   image. **Use the recommender's `thumbnail_url` field verbatim.** It is
   workspace-relative by default (portable — resolves against the workspace on any
   runtime). On a host that serves workspace files over HTTP (e.g. SwarmAI's daemon), set
   `$POLLINATE_THUMB_URL_PREFIX` (e.g. `http://localhost:18321/api/workspace/file/raw?path=`)
   and the recommender prefixes it automatically. Do NOT hand-build a daemon URL. The
   `thumbnail_url` looks like:
   `.artifacts/pollinate/assets/deck-styles/<slug>.png` (relative), or the prefixed
   absolute form when `$POLLINATE_THUMB_URL_PREFIX` is set. Example (relative):
   ```
   Based on your answers, three styles fit:

   ![vellum](.artifacts/pollinate/assets/deck-styles/vellum.png)
   **Vellum** — scholarly, quiet navy + warm-yellow serif. Fits a considered leadership brief.

   ![blue-professional](.artifacts/pollinate/assets/deck-styles/blue-professional.png)
   **Blue-Professional** — consulting-grade cream + cobalt. Clean and authoritative.

   ![cartesian](.artifacts/pollinate/assets/deck-styles/cartesian.png)
   **Cartesian** — museum-catalog Playfair serif. Calm and editorial.
   ...
   ```
   (The recommender's `thumbnail` field is always the workspace-relative path;
   `thumbnail_url` adds `$POLLINATE_THUMB_URL_PREFIX` only if that env var is set.)
