...
def validate_delivery(content_dir: str) -> dict:
    ...
    # ── Check 2: QR code image present (OPT-IN — DDD-configurable) ─────────
    # SwarmAI required a QR in every deliverable; a portable DDD may not want one.
    # Only enforced when POLLINATE_REQUIRE_QR=1 — otherwise auto-passes (no brand-lock).
    if os.environ.get("POLLINATE_REQUIRE_QR") == "1":
        ...
        if qr_files:
            ...
            errors.append("MISSING: QR code image — no qr-*.png (POLLINATE_REQUIRE_QR=1)")
    else:
        checks_passed += 1  # not required for a portable DDD

    # ── Check 3: publish link in delivery text (OPT-IN — DDD-configurable) ──
    # Was a hard github.com requirement (SwarmAI publishing convention). Now opt-in:
    # set POLLINATE_REQUIRE_LINK to the substring your DDD requires (e.g. your domain),
    # else auto-passes. "1" preserves the legacy github.com behavior.
    _link_req = os.environ.get("POLLINATE_REQUIRE_LINK", "")
    if _link_req:
        needle = "github.com" if _link_req == "1" else _link_req.lower()
        link_found = False
        for f in list(root.glob("*.md")) + list(root.rglob("*.html")):
            try:
        ...
        if link_found:
            ...
            errors.append(f"MISSING: required link '{needle}' (POLLINATE_REQUIRE_LINK)")
    else:
        checks_passed += 1  # not required for a portable DDD
