...
- [decision] **EVOLUTION.md is wired into `ddd_entry_lifecycle` as DEDUP-ONLY + FULLY evergreen for age-decay — this is DELIBERATE, not an omission** (2026-08-08, run_2816ab1c) — `EVOLUTION_EVERGREEN_SECTIONS` lists ALL 7 sections, so `assess_decay`/`reclaim_noise_entries` are intentional no-ops on EVOLUTION; `_run_evolution_lifecycle` runs ONLY the exact-dup dedup sweep (still invokes decay/reclaim as cheap no-ops to keep the code path identical to MEMORY/KNOWLEDGE + self-document that decay is inert-by-choice). **Why (Principle 1 — value, not age):** the "Optimizations Learned" O-entries are distilled operational wisdom (O030 disaster-recovery doctrine, O025 know-your-runtime), carry NO `created_date` metadata (only prose dates) AND EVOLUTION has no usage→ref bridge — so age-decay would treat every O-entry as infinitely-old ref:0 noise and strip them ALL, burying hard-won judgment because a counter never ticked. Size control for EVOLUTION = `fold_corrections` (owns the huge Corrections section) + dedup, NEVER age-death. First AC6 apply (before this fix) archived O029 wrongly — that was the signal the section was mis-classified as decayable. Anyone adding age-decay to EVOLUTION must know this is a considered decision.
...
### TTFT (Time-To-First-Token) — instrumentation exists; the bottleneck is model prefill, NOT cold-start (code-traced + log-verified 2026-08-16)
<!-- maturity: sparse | sources: 3 | verified: true | used: false | days: 0 | trust: high | promoted: none | source:manual -->

- [model] **The end-to-end TTFT probe ALREADY EXISTS and emits on every turn — do NOT rebuild it** (C049/P9: a pipeline was opened to "add" it and killed at Gate-0 when the source proved it present). Two complementary probes, wired and live:
  - **TTFT probe** (`session_router.py`: `_format_ttft_line` ~:1041, `_ttft_t0` ~:2240 at true per-turn entry BEFORE slot+recall, `_ttft_presend_ms` ~:2667 marks the router/model boundary, first-token latch ~:2709-2723; `run_ad19fd5b`/`run_332ccfd1`). Measure-only (broad try/except so it can never break the stream). Emits `TTFT=…ms (first-token=…) | slot=… recall=… pre_send=… send+infer=… [retries=…]`. First-token anchor = `_TTFT_FIRST_CONTENT_TYPES = (text_delta, thinking_delta)` — thinking_delta is deliberately included (Opus adaptive thinking usually streams a thinking token FIRST, so a text-only anchor reads systematically LATE). ⚠️ The anchor is NOT the `is_first_message` flip (streaming_orchestrator ~:815) — that fires on the first SDK message incl. SystemMessage/init, not model output.
  - **spawn_perf** (`session_unit.py` ~:3074, `run_924de37c`): covers ONLY the cold-start subprocess-spawn segment, split into `lock_wait / configure / wrapper_aenter / total`. `wrapper_aenter` (start CLI subprocess + MCP handshake) is the whole spawn cost.
- [model] **TTFT splits into two segments (how to measure, per R30#4 — never trust a single log line):** `grep 'TTFT=' backend-daemon.log`, then group by `pre_send` vs `send+infer` (and pair with `spawn_perf` on the same session to tell cold from warm). Findings (qualitative — re-measure for live numbers):
  - **`pre_send`** = OUR work (slot-queue + user-msg DB persist + prompt assembly + recall injection) = the SMALL half, sub-second median. Not the culprit.
  - **`send+infer`** = SDK send + (cold-only) subprocess spawn + Bedrock prefill to first token = the LARGE half, ~10s-scale median, long tail into 100s+.
- [guideline] **Durable tell — "cold-start / subprocess spawn is the TTFT bottleneck" is the WRONG frame (the reflex to correct).** Log-verified: cold starts (rows with a paired `spawn_perf`) are a TINY minority of turns, yet slow turns (large `send+infer`) are ~half — WARM turns (process already up, no spawn) routinely take tens of seconds too. The same session swings fast→slow with process and code unchanged; what varies is that turn's prefill context size + Bedrock load. So the TTFT tail is **Bedrock-side prefill (largely uncontrollable)**; the 10-16s `wrapper_aenter` only explains the cold-start minority. Corollary: optimizing subprocess-spawn / thread-pools shows NO effect on general TTFT (wrong layer). The real lever for "feels faster" is NOT shrinking TTFT but stopping the frontend from misreading a long prefill as "stuck / backend offline" — that belongs to the SSE 90s-stall / heartbeat investigation (paused `run_c54ae2db`), not to TTFT. Reflex on any "TTFT/new-session got slower" claim: grep the `TTFT=` lines, group pre_send vs send+infer, pair cold/warm via spawn_perf — never infer causality from one line or a feeling, never assume cold-start is the general cause.
...
**Reconcile fetch = newest-`RECONCILE_TAIL` tail, NOT full history (shipped 2026-08-06, run_acaa53ea).** The 6 turn-end reconcile sites in `useChatStreamingLifecycle.ts` fetch only the newest `RECONCILE_TAIL=50` rows via a dedicated `getSessionMessagesReconcileTail`, not the full session (was O(N) pull + full `toMessageCamelCase` per turn on 1000+ row sessions). Two correctness facts a future maintainer MUST NOT break: **(1) `RECONCILE_TAIL=50` counts RAW DB ROWS, not merged bubbles, and safety comes from the MATCH KEYS, NOT from the number 50.** The backend persists ONE row per `assistant` SSE event (crash-safety); the `GET .../messages` endpoint's `_merge_consecutive_assistant_messages` folds consecutive rows on read and stamps the merged bubble with the **FIRST-IN-RESULT row's id** (`chat.py:1081`). So a tail that cuts mid-turn (returns rows A3..A5 instead of A1..A5) yields a merged bubble whose id DIFFERS from the full-load's (A3, not A1). **This does NOT duplicate the bubble — because reconcile correlation is NOT id-based**, so the id change is irrelevant: (a) the main send path persists the SAME `{client_id}-asst` on EVERY row of the turn (`session_router.py:2285`), and the local bubble's id is `local-{client_id}-asst` (never renamed to a DB id), so `_applyMerge`'s **client_id fallback** (`MessageStore.ts:825-826`) matches regardless of the merged row id; (b) the continuation paths (`continue_with_answer`/`continue_with_permission`) persist NO client_id and use a numeric placeholder id, so the **H2 backstop** (`MessageStore.ts:876-880`: drop a numeric-id assistant placeholder once a real DB assistant row exists) finalizes to one bubble — a mid-turn cut still returns real rows, so `hasRealAssistant` holds. Both paths regression-tested (`MessageStore.reconcileChurn.test.ts` "MID-TURN CUT" ×2, each mutation-verified RED when its guard is disabled). Because the guard is client_id-replication + H2, **50 is a perf floor, not a correctness boundary — but do NOT reason that a LARGER tail is "safer": the cut can land mid-turn at any tail size, so raising 50 changes nothing about correctness (it's the match keys that protect it), and lowering it only wastes the perf win.** A cold initial-load (200-cap) always restores the full turn anyway. **(2) The reconcile fetch deliberately BYPASSES the ETag cache** (`getSessionMessagesReconcileTail` sends no `If-None-Match`, writes no `_messageEtags` slot) — because the backend ETag `"session:count"` is limit-agnostic, so a capped fetch sharing the full-history cache slot would 304-truncate a later initial-load. The cache slot is owned exclusively by initial-load. (See IMPROVEMENT.md § What Failed for the full poisoning-chain lesson.)

### Canvas Surfacing — how a file reaches Canvas (READ THE MAP BEFORE TOUCHING)

Canvas auto-surfacing is a **recurring regression site** (the `relevance`-gate kill
run_1e39d21e, the surface-vs-run-report ordering run_f1fbf37d, the dual-resolver
near-miss, close-first-then-dispatch). **Before touching ANY Canvas auto-surface /
`file_changed` / rail / auto-pop / `open-file` logic, read the authoritative E2E map:**
`Projects/SwarmAI/2-understanding/knowledge/designs/2026-08-09-canvas-surfacing-e2e-flow.md`
— it has the full pipe, both admission gates, the 7 traps, and a grep-able symbol index
(no line numbers by design — they drift; grep the symbol).

**The pipe (one line):** backend `needs_human_review` decides `kind` → `_build_file_write_events`
emits a `file_changed` SSE event → `useChatStreamingLifecycle` re-dispatches a
`swarm:file-changed` window event → **two independent consumers** (`useReferencedFiles`
= the rail list; `useCanvasAutoSurface` = auto-pop) → auto-pop dispatches `swarm:open-file`
→ `useCanvasHost.handleOpenFile` resolves the path + sets the active file (panel
auto-opens). Three entry points converge on this pipe: mid-run write, pipeline-finish
batch (`surface_run_outputs`/`build_surface_events`), and manual/agent `open-canvas-file`.

**THE INVARIANT (hold this or you get a Trap-1 regression):** `kind` (backend verdict)
is the **SOLE surfacing authority**. `relevance` is **legacy-fallback-only, active
exclusively when `kind === undefined`** (old backend). Any frontend gate that consults
`relevance` while `kind` is present will kill every kind-whitelisted write — because the
write-emit path sends `kind` but NO `relevance`, so the SSE bridge fail-closes it to
`'incidental'`. That exact stacking bug (a new authoritative gate left ANDed with an
unconditional legacy one) was run_1e39d21e. The rail and auto-pop are DIFFERENT gates —
verify each independently; they do not break together.
...
- [model] **Canvas = FileViewerPanel + CanvasOutputRail, output-triggered & fully tab-scoped** — replaced SwarmRadar. **Signal chain:** agent Write/Edit → `streaming_orchestrator.get_tool_category`→`write` → `MergedToolBlock` dispatches `swarm:file-referenced {path, operation:'written', sessionId}` → `useCanvasAutoSurface` (debounce-coalesce + gentle suppression) → `swarm:open-file` → `ThreeColumnLayout` handler → `setFileViewerFile` → content-adaptive view (renderable→Preview via `HtmlRenderer` `<iframe sandbox="allow-scripts allow-popups">`; modified→Diff; new→Source). **Output list** = `useReferencedFiles(sessionId).grouped.written` MINUS bookkeeping (`isBookkeepingPath`: denylist `.artifacts`/`.git`/`.context` + dotfile-basename + temp) + `useChangeStatus` NEW/UPD git badge (both hooks in `hooks/`, NOT Radar-owned). **Tab-scoping is the key invariant, and it keys on `tabId` EVERYWHERE (run_26aa6caa — was split-brained on `sessionId`):** all Canvas state is per-active-tab. `activeTabId` (published into `ActiveSessionMeta`/`SessionMetaContext` by ChatPage) is the ONE scope key across the whole surface — the file/pin state (`useCanvasHost`), the outputs rail (`useReferencedFiles`), the auto-surface filter (`useCanvasAutoSurface`), AND the SSE `swarm:file-changed` stamp all use it. **NEVER `sessionId`** — it flips undefined→resolved on a new tab's first turn (exactly when files get written), so a sessionId-stamped event went out UNDEFINED and consumers' fail-open filter bled the write into every mounted rail (the P0 that run_26aa6caa fixed). The stamp is the stream handler's CAPTURED tab (`createStreamHandler(id, capturedTabId)`), not whatever tab is active at dispatch — so a BACKGROUND tab's reconnect/resend (via `retryStreamFn`) stamps ITS OWN tab, not the foreground (the R27 half-migration sibling, also closed in run_26aa6caa: all 4 retry handlers use `capturedTabIdForRetry`). `shouldResetCanvasOnTabChange(prev,next) = prev!==undefined && next!==prev` (pure, unit-tested); on real switch → clear file+pin/mute/collapsed. Consumers keep an event only when its owning `tabId` matches the tab their rail is mounted for; fail-open ONLY when truly unstamped (the single-tab pre-registration window, where one rail is mounted so there's no second rail to bleed into). **Controls:** pin (file-scoped) / mute (session-scoped) / expand / the header × now COLLAPSES to a side rail (`collapseToRail`, run_26aa6caa — "close 不是关闭 而是 collapse"; never unmounts — true unmount survives only via the last-file-tab-closed path inside FileViewer); LeftSidebar `Canvas` card → `swarm:open-canvas` for file-less open. **SwarmRadar removed:** 3 sections replaced (Changes→Canvas, Attention→ChatHeader `AlertsPill` via shared `useRadarAttention`, Jobs&Runs→left-nav overlay); KEPT `AttentionList`(AlertsPill)/`HistoryView`/`useRadarAttention`/`types.ts`. (2026-08-02)
  <!-- ref:0 | last:none | decay:active | source:manual -->
...
- [principle] **async-blocking-on-event-loop is THE signature trap of this stack — close it with an AST gate + baseline ratchet, not vigilance** — The backend is a single-process FastAPI + one asyncio event loop feeding, simultaneously: every chat tab's SSE token stream, every HTTP API, channels, and jobs. Any synchronous blocking call on an `await` path — a sync `sqlite3.connect` session, an `rglob` full-tree walk, a large-file read, a tree-sitter graph build — MONOPOLIZES the whole loop; for its entire duration EVERY tab's streaming and EVERY request freeze together. It is not "that one request is slow," it is "the whole app is frozen." **Defense = an AST gate** (`backend/tests/test_router_async_blocking.py`): it scans every `.py` under `SCOPE_DIRS`, forbidding `_BLOCKING_DOTTED` (incl. `sqlite3.connect`) inside an `async` body; a baseline ratchet stops NEW violations while the existing stock is cleared in batches; `SCOPE_DIRS` only widens to the next dir AFTER the current one is clean (widening early turns the gate RED — the file warns about this anti-pattern itself). **Fix shape:** lift the sync logic into a `_read()`/`_work()` helper dispatched via `asyncio.to_thread`, wrap the AWAIT in the try/except so degrade-to-empty and 500 paths behave identically; fail-fast rejection guards (400s, info-leak/security checks) deliberately STAY in the async body. This is P7 ("defense outside the agent") applied at the performance layer — vigilance alone never caught it (I held O006/O020 + a recall signal and still missed it, 2026-08-10). Stock cleanup spans multiple runs (core/ + a large silent-swallow backlog); the live counts live on the record in the test file, not here (they drift). (2026-08-10, refs 6495faaa/039c4f32, source:manual)
  <!-- ref:0 | last:none | decay:active | source:manual -->
- [principle] **recall legs are CPU-bound → adding threads is ~1x; the only lever is LESS CPU** — `recall_all`'s legs (especially the ddd leg's BM25 tokenization over ~MB-scale markdown) are CPU-bound. A `ThreadPoolExecutor` over the legs measures ≈1.14x — the GIL eats the parallelism, and it nests threads inside the asyncio executor thread `recall_all` already runs in. So the real optimization is to REDUCE CPU, not add threads: `_ddd_section_scores_multi` and `_ddd_entry_hits` each independently parsed the SAME corpus (tokenizing it TWICE per recall) → `_parse_docs_sections()` parses once and both legs share it. This CORRECTS run_16113a9b's two recorded-but-false premises ("~920ms serial 4-leg sum" and "leg concurrency rejected for io-pool starvation") — rejecting concurrency was right, but the reason is "it wouldn't work (CPU/GIL)," NOT starvation. Before ever reaching for a thread pool on recall, re-read this. Live measurements (warm total, per-leg ms, speedup) live in the `recall_all` docstring — the place the next person tempted to add threads will read — not frozen here. (2026-08-10, ref 84544573, source:manual)
  <!-- ref:0 | last:none | decay:active | source:manual -->
...
## Conventions
<!-- maturity: sparse | sources: 3 | verified: false | used: false | days: 0 | trust: high | promoted: none -->
- [guideline] A diagnostic/telemetry count MUST equal the ACTION it describes, not a wider gauge. `compute_entry_noise` reports a WIDE reclaimable count (ref==0 + past-grace + dormant/archived) while the real `reclaim_noise_entries` (keep-class + evergreen filtered) strips fewer/zero — so a UI showing "5 reclaimable" while a sweep removes 0 is a lie. Panel reclaimable/duplicate counts must use dry_run reclaim-candidate counts (gate==action), the same predicate the sweep uses. This is the honesty principle the whole run addressed (also the context-token disk-vs-injected口径). (2026-08-08, run_2816ab1c)
  <!-- ref:0 | last:none | decay:active | source:manual -->
...
## Runtime Traps
<!-- maturity: sparse | sources: 2 | verified: false | used: false | days: 0 | trust: high | promoted: none -->
- [pitfall] **Verify deployed-vs-undeployed by reading the daemon's runtime `.version` — NEVER trust a narrated build-point (incl. another agent's self-report)** (2026-08-10) — The daemon writes its live build to `~/.swarm-ai/daemon/.version` in the form `<semver> <sha> <timestamp>` (e.g. `1.28.1 90ec83e0 2026-08-10 13:20:32`). That sha is the ONLY source of truth for "what users are actually running." The deploy/undeploy predicate is `git merge-base --is-ancestor <commit> <daemon-sha>` (ancestor ⇒ deployed). **Why this entry exists:** a parallel agent claimed the daemon was built from `bcec9d4f` (previous night) and concluded "users get ~zero benefit, ~32 commits undeployed" — the runtime `.version` actually showed `90ec83e0` (rebuilt the SAME day), which INVERTED the conclusion: all the async/observability/security/recall work was already live; only 3 commits were genuinely undeployed. A narrated build-point is a *claim to verify*, not a fact — the `.version` file settles it in one read. (Do not store the counts/SHAs — they drift daily; the method is the durable part.) (source:manual)
