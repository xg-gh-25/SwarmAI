...
- **Brain-centric UI — the paradigm made visible (2026-08-12)** — the desktop nav was re-architected from a file-explorer + tool-box (always-visible SwarmWS tree + SwarmRadar sidebar) into a **brain organ panel**, the UI expression of "DDD = a universal brain + 0..N governed assets". Three regions, BRAIN first (mint-green, the sole differentiator): **Brain-Hub** presents every project as a *brain* with the six-section cognitive structure (ONE builder, TWO projections, zero stored metrics); **Canvas** is the single output surface where agent deliverables auto-surface classified by KIND (content/knowledge pop, code aggregates into one finish-time PR, noise dropped) — it replaced SwarmRadar's 3 sections (→ Canvas / `needs-you` / `jobs`); **TSCC** (Thread-Scoped Cognitive Context) makes each thread's REAL injected cognition inspectable (recall hits+scores, per-file token budget, security scan) — honest telemetry, never fabricated. SwarmWS was NOT deleted — it migrated from an always-visible column to an on-demand "Workspace" overlay. The agent has PROPRIOCEPTION over these surfaces (`ui_action` → `ALL_SHOW_EVENTS` allowlist; nav-only surfaces like hive/settings are a security boundary the agent cannot open). Full architecture + invariants: `knowledge/designs/2026-08-12-brain-centric-ui-architecture.md`.
