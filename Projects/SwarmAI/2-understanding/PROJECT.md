...
**Tab-concurrency work — CURRENT STATE SNAPSHOT (2026-08-02, for fast triage if a tab/OOM issue appears):**
- [x] **Ceiling 4→6 SHIPPED to git** — commit `795e85a5`, in HEAD. `_MAX_TABS_CEILING=6` (backend) + `MAX_TABS_HARD_CEILING=6` (frontend). Verify: `git show HEAD:backend/core/resource_monitor.py | grep _MAX_TABS_CEILING`.
- [ ] **⚠️ NOT DEPLOYED** — the running daemon binary predates the change (verify: `stat -f '%Sm' ~/.swarm-ai/daemon/python-backend` vs the commit time; if binary older → still ceiling=4 live). Needs `s_swarm-build` + daemon restart (destructive, XG approval) to take effect. **So "how many tabs can I open" = still 4 until deployed, 6 after.**
- [x] **Aggregate-spike test SHIPPED** — commit `eea70760`, `TestSpawnBudgetAggregateSpike` in `test_process_resource_management.py`. This is the FAIL-LOUD alarm: `test_large_machine_penalty_is_structurally_weak` will go red if anyone changes the spawn-cost model and alters large-machine behavior.
- [ ] **~64 commits ahead of origin/main, NOT pushed** (verify: `git status -sb`). Push is XG-initiated (STEERING #5).
- [x] **B (close the aggregate-spawn-spike window) — DECIDED NO-GO, zero code shipped** (run_c1313394 cancelled clean; verify no residual: `git status --porcelain backend/core/resource_monitor.py backend/core/session_unit.py backend/core/lifecycle_manager.py` = empty). Reason: COE05-adjacent peak-OOM code; Gate-0 found my minimal fix would double-count (charge ~2100·N for ~1500·N real → re-block legit tabs) and that the scary streaming pileup is already bounded by `MAX_CONCURRENT_STREAMS=3`. The residual risk (only ~1.5GB×N steady accumulation, in a ~60s MCP-load window, on 36-64GB machines, requires rapid-fire opening 5-6 tabs) is UNMEASURED — decided not to touch the landmine without canary data.
- **IF A TAB/OOM ISSUE ACTUALLY APPEARS** (triage order): (1) is it deployed? (binary mtime vs commit); (2) which gate fired? — `spawn_budget` deny (RAM, session_router acquire log "spawn budget denied") vs `_await_streaming_slot` 120s wait (silent, session_unit log "stream_admit_wait") vs a real jetsam/exit-9 (the aggregate-spike window — check for rapid multi-spawn just before); (3) the 3-gates model + the penalty structural-weakness are in TECH § Session System + IMPROVEMENT § What to Watch For; (4) if it IS the aggregate spike → that's when B becomes worth building (do the sample-on-spawn / replace-not-stack version Gate-0 outlined, NOT the naive reserve). Full study: `Knowledge/Library/2026-08-02-concurrent-tab-session-resource-models.md`.
...
- 2026-08-21: **DEFER — "知识的质疑/演化过程可追溯进脑"(接活 knowledge_graph 的 conflicts/supersede 边),先量再决定** — [decision] 借鉴 AIDLC Collaborative(`aws-samples/sample-collaborative-ai-dlc`)的"discussion 进 graph"机制内核(**不是**其 Yjs 多人实时协作层——那是多用户产品命题,对单人 SwarmAI 拓扑+命题双重错配,两轮 dive 已 NO-GO)。想法:补 3 个 predicate(`refuted_by`/`questioned_by`/`revised_from`)+ 接 3 个生产者(correction 落 EVOLUTION→图 / Gate-2 证伪 finding→图 / supersede 旧条目→图)+ recall 2-hop 带出争议史,治 CLASS-A/B(把可能过时结论当圣旨)。**战略满分**(ROI 3.15;战略对齐 5/5,命中 T1 memory-moat + T3 understanding>execution),**但 DEFER**,两个红线:(1) **历史三连负信号**——IMPROVEMENT.md `2666`(Brain Hub 知识图谱 NO-GO:"价值全在 supersession 边,但真实数据 0 边")、`63`(Run4 DEFER:"对未观测问题造机制")、可复现 tell "在关系上造机制前先验证关系在真实数据被填充" —— 现状 `conflicts_with=0`、质疑关系从无生产者,提的方案正是那三条警告的 C042 家族陷阱;(2) **未解的实体对齐**——"这条 correction 质疑的是哪条已有知识条目"没有稳定 ID(EVOLUTION↔MEMORY↔DDD 条目间无 key),唯一现存生产者用脆弱 title 子串匹配,错连比不连更毒。**复活路径(GO 的唯一诚实前提)**:先做**一周纯日志埋点**(无写图、零风险)——在三处只记不写,数真实一周产生多少条"可对齐到某条已有知识"的质疑/取代事件。判据:≥两位数且可对齐 → 重开 GO(那时接线有真数据托底);个位数或对不齐 → 确认单人产品结构性稀疏,永久 REJECT 此形态,改由更轻方式(如 correction 条目内联"取代了 X")承载。**硬边界**:永不 Yjs/实时/多人层;永不 embedding 语义对齐(PRODUCT.md 非目标:纯 FTS5)。(2026-08-21, s_evaluate)
  <!-- ref:0 | last:none | decay:active | source:manual -->
- 2026-08-02: **Concurrent-streaming cap HELD at `MAX_CONCURRENT_STREAMS=3`, NOT raised to 4 (evidence-based, run_6e3d0f2a follow-up)** — [decision] Standing "don't re-litigate 3→4". Rationale: 4 concurrent streams worst-case approaches the 90% RAM gate on a 36GB machine (per-session streaming peak lives in `session_unit.py` comments — not stored here, it drifts). 3 is the documented COE05 peak-OOM floor (commit `8f211839`), deliberately split from the "how many sessions may EXIST" spawn_budget. **If concurrency is ever raised, the RIGHT move is to make `MAX_CONCURRENT_STREAMS` DYNAMIC (scale with RAM, like `compute_max_tabs` already does), NOT bump the constant 3→4** — a fixed constant ignores machine size (16GB and 64GB both get 3 today). See TECH § Session System (three-gates model) for the full mechanism, and IMPROVEMENT § What to Watch For for the penalty-factor weakness that a dynamic cap would also need to address.
  <!-- ref:0 | last:none | decay:active | source:manual -->
- 2026-08-01: **Chat tab ceiling raised 4→6 (option A, run_a7252400, shipped commit `795e85a5`)** — [decision] `_MAX_TABS_CEILING` (backend) + `MAX_TABS_HARD_CEILING` (frontend) both 4→6 so a 36GB machine offers 5 chat + 1 channel. Chose option A (bump ceiling only) over B (also raise `MAX_CONCURRENT_STREAMS`, rejected — virtual benefit, real peak-OOM) and C (drop the hard ceiling, less predictable UX). **Peak/抢资源 gates deliberately untouched:** `_DEFAULT_SPAWN_COST_MB=1200`, 90% RAM threshold, `MAX_CONCURRENT_STREAMS=3` — because the ceiling is only a frontend UX cap and backend admission is gated solely by `spawn_budget()` (see TECH § Session System). Small machines auto-degrade via the RAM formula (new `TestCeilingAutoDegradeOnLowMemory`, mutation-verified non-vacuous). En route, corrected a long-stale spawn-cost comment (measured cost is main-process RSS ~300-500MB, not the "verified 1400-1600MB tree RSS" old comments claimed) and synced 4 hardcoded ceiling=4 test sites (R7 same-change scan). Committed automatically per STEERING #5, stopped at PUSH-READY (not pushed).
  <!-- ref:0 | last:none | decay:active | source:manual -->
