...
describe('FileViewerStatusBar — file-operation cluster', () => {
  ...
  // ── close is NOT in the status bar anymore (run_f49d3ff3 R2) ──
  // Close moved to FileViewer's unified file-chrome header (one type-agnostic
  // affordance for all types). The status bar is info + copy-path/attach only.
  it('never renders a close button (close moved to the unified file-chrome header)', () => {
    render(
      <FileViewerStatusBar fileName="deck.html" fileSize={100} viewType="html-preview" filePath="/ws/out/deck.html" />,
    );
    expect(screen.queryByTestId('statusbar-close')).toBeNull();
    // copy-path/attach still present when filePath is set
    expect(screen.getByTestId('statusbar-copy-path')).toBeTruthy();
  });
});
