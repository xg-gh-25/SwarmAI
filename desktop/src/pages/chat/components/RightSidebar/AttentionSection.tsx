 ...
 * waiting tabs). Each card carries a category tag pill (PIPELINE / JOB / TAB) so
 * the three kinds are distinguishable at a glance, plus a kind emoji.
 *
 * Click semantics by kind:
 ...
 * disappears when the next poll (useRadarAttention, 30s) sees the source
 * condition resolved. To give immediate feedback in that window, a clicked item
 * enters an "acting" state (greyed out + action text → resuming…/opening…/
 * switching…). Acting is section-level state (a Set of item keys) with a 35s
 * auto-expiry — longer than the 30s poll so a successfully-actioned item stays
 * visually "acting" right up until the poll removes it (no confusing un-acted
 * flash), while a clicked-but-abandoned item (user didn't send the injected
 * message) clears itself after the poll re-confirms it's still pending.
 *
