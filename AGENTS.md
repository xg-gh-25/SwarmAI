...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 14,435 symbols, 20,736 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 2008+ | `git log --oneline | wc -l` |
| Duration | ~116 days | First commit to latest (1 human contributor) |
| Backend core modules | 130 Python files, 61204 LOC | `find backend/core -name "*.py" -exec cat {} + | wc -l` |
| Total backend LOC |  | `find backend -name "*.py" -not -path "*/.*" -not -path "*/__pycache__/*" | xargs cat | wc -l` |
| Test files | 272 | `find backend/tests -name "*.py" | wc -l` |
| Skills (agent capabilities) | 86 | `ls -d backend/skills/s_* | wc -l` |
| Post-session hooks | 14 | `ls backend/hooks/*.py | wc -l` |
| React components | 173 | `find desktop/src -name "*.tsx" | wc -l` |
| Pipeline spec depth | 1350 lines | `wc -l backend/skills/s_autonomous-pipeline/INSTRUCTIONS.md` |
| Largest state machine | 2802 lines | `wc -l backend/core/session_unit.py` |
