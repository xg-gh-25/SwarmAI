...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 20,531 symbols, 33,137 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 2882+ | `git log --oneline | wc -l` |
| Duration | ~145 days | First commit to latest (1 human contributor) |
...
| Total backend LOC | 201656 | `git ls-files '*.py' | grep '^backend/' | grep -v '/tests/' | xargs wc -l | awk '$2!="total"{n+=$1} END{print n}'` |
