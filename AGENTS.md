...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 13,883 symbols, 14,663 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 1921+ | `git log --oneline | wc -l` |
| Duration | ~115 days | First commit to latest (1 human contributor) |
| Backend core modules | 126 Python files, 59178 LOC | `find backend/core -name "*.py" -exec cat {} + | wc -l` |
| Total backend LOC | 261042 | `find backend -name "*.py" -not -path "*/.*" -not -path "*/__pycache__/*" | xargs cat | wc -l` |
| Test files | 256 | `find backend/tests -name "*.py" | wc -l` |
| Skills (agent capabilities) | 86 | `ls -d backend/skills/s_* | wc -l` |
| Post-session hooks | 14 | `ls backend/hooks/*.py | wc -l` |
| React components | 171 | `find desktop/src -name "*.tsx" | wc -l` |
| Pipeline spec depth | 1312 lines | `wc -l backend/skills/s_autonomous-pipeline/INSTRUCTIONS.md` |
| Largest state machine | 3596 lines | `wc -l backend/core/session_unit.py` |
