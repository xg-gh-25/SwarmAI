...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 14,909 symbols, 21,453 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 2151+ | `git log --oneline | wc -l` |
| Duration | ~120 days | First commit to latest (1 human contributor) |
| Backend core modules | 135 Python files, 64455 LOC | `find backend/core -name "*.py" -exec cat {} + | wc -l` |
| Total backend LOC |  | `find backend -name "*.py" -not -path "*/.*" -not -path "*/__pycache__/*" | xargs cat | wc -l` |
| Test files | 286 | `find backend/tests -name "*.py" | wc -l` |
...
| Largest state machine | 3182 lines | `wc -l backend/core/session_unit.py` |
