...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 17,986 symbols, 26,922 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 2740+ | `git log --oneline | wc -l` |
| Duration | ~137 days | First commit to latest (1 human contributor) |
| Backend core modules | 134 Python files, 72715 LOC | `git ls-files '*.py' | grep '^backend/core/' | grep -v '/tests/' | xargs wc -l | awk '$2!="total"{n+=$1} END{if(n>0) print n}'` |
| Total backend LOC | 173167 | `git ls-files '*.py' | grep '^backend/' | grep -v '/tests/' | xargs wc -l | awk '$2!="total"{n+=$1} END{print n}'` |
