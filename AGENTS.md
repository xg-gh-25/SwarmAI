...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 13,580 symbols, 14,387 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 1883+ | `git log --oneline | wc -l` |
| Duration | ~108 days | First commit to latest (1 human contributor) |
| Backend core modules | 121 Python files, 56211 LOC | `find backend/core -name "*.py" -exec cat {} + | wc -l` |
| Total backend LOC | 255125 | `find backend -name "*.py" -not -path "*/.*" -not -path "*/__pycache__/*" | xargs cat | wc -l` |
| Test files | 252 | `find backend/tests -name "*.py" | wc -l` |
| Skills (agent capabilities) | 86 | `ls -d backend/skills/s_* | wc -l` |
| Post-session hooks | 14 | `ls backend/hooks/*.py | wc -l` |
| React components | 169 | `find desktop/src -name "*.tsx" | wc -l` |
| Pipeline spec depth | 1206 lines | `wc -l backend/skills/s_autonomous-pipeline/INSTRUCTIONS.md` |
| Largest state machine | 3489 lines | `wc -l backend/core/session_unit.py` |
