...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 13,924 symbols, 14,708 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 1928+ | `git log --oneline | wc -l` |
| Duration | ~115 days | First commit to latest (1 human contributor) |
| Backend core modules | 126 Python files, 59295 LOC | `find backend/core -name "*.py" -exec cat {} + | wc -l` |
...
| React components | 172 | `find desktop/src -name "*.tsx" | wc -l` |
| Pipeline spec depth | 1312 lines | `wc -l backend/skills/s_autonomous-pipeline/INSTRUCTIONS.md` |
| Largest state machine | 3619 lines | `wc -l backend/core/session_unit.py` |
