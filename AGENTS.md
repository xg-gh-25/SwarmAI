...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 20,556 symbols, 33,233 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 2881+ | `git log --oneline | wc -l` |
| Duration | ~144 days | First commit to latest (1 human contributor) |
| Backend core modules | 136 Python files, 75818 LOC | `git ls-files '*.py' | grep '^backend/core/' | grep -v '/tests/' | xargs wc -l | awk '$2!="total"{n+=$1} END{if(n>0) print n}'` |
| Total backend LOC | 201655 | `git ls-files '*.py' | grep '^backend/' | grep -v '/tests/' | xargs wc -l | awk '$2!="total"{n+=$1} END{print n}'` |
| Test files | 384 | `find backend/tests -name "*.py" | wc -l` |
