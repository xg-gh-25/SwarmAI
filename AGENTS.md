...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 13,387 symbols, 14,241 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 1799+ | `git log --oneline | wc -l` |
| Duration | ~105 days | First commit to latest (1 human contributor) |
| Backend core modules | 121 Python files, 56064 LOC | `find backend/core -name "*.py" -exec cat {} + | wc -l` |
| Total backend LOC | 251605 | `find backend -name "*.py" -not -path "*/.*" -not -path "*/__pycache__/*" | xargs cat | wc -l` |
| Test files | 249 | `find backend/tests -name "*.py" | wc -l` |
| Skills (agent capabilities) | 86 | `ls -d backend/skills/s_* | wc -l` |
| Post-session hooks | 14 | `ls backend/hooks/*.py | wc -l` |
| React components | 167 | `find desktop/src -name "*.tsx" | wc -l` |
| Pipeline spec depth | 1200 lines | `wc -l backend/skills/s_autonomous-pipeline/INSTRUCTIONS.md` |
| Largest state machine | 3444 lines | `wc -l backend/core/session_unit.py` |
