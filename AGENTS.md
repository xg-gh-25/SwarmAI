...
| Code Intelligence (AST graph) | `backend/core/code_intel/__init__.py` | 15,658 symbols, 22,055 edges — deterministic graph traversal for code context retrieval |
...
|--------|-------|---------------|
| Total commits | 2273+ | `git log --oneline | wc -l` |
| Duration | ~123 days | First commit to latest (1 human contributor) |
| Backend core modules | 139 Python files, 68404 LOC | `find backend/core -name "*.py" -exec cat {} + | wc -l` |
| Total backend LOC | 292969 | `find backend -name "*.py" -not -path "*/.*" -not -path "*/__pycache__/*" | xargs cat | wc -l` |
| Test files | 310 | `find backend/tests -name "*.py" | wc -l` |
| Skills (agent capabilities) | 86 | `ls -d backend/skills/s_* | wc -l` |
| Post-session hooks | 14 | `ls backend/hooks/*.py | wc -l` |
| React components | 179 | `find desktop/src -name "*.tsx" | wc -l` |
| Pipeline spec depth | 1385 lines | `wc -l backend/skills/s_autonomous-pipeline/INSTRUCTIONS.md` |
| Largest state machine | 3791 lines | `wc -l backend/core/session_unit.py` |
| Context system | 1132 lines | `wc -l backend/core/context_directory_loader.py` |
